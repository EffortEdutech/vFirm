...
- Status (2026-09-06): Authorized and executed. OP-H1 through OP-H5 existed from 2026-09-03 but predated AWIA virtual staff and covered a human-only pilot day; OP-H3 and OP-H4 were extended to genuinely rehearse an AWIA virtual staff member in each firm's daily workload (see ADR-070 in `DECISION_REGISTER.md` and the OP-H3/OP-H4/OP-H5/OP-H6 addenda). OP-H6's technical recommendation is `GO_FOR_CONTROLLED_MULTI_FIRM_PILOT_OPERATIONS_ACCEPTANCE`; the product owner accepted OP readiness 2026-09-06 (ADR-071 in `DECISION_REGISTER.md`). **Phase C is CLOSED.**
- Status (2026-09-06): Release 4 (R4-S1 through R4-S6) was already built and accepted 2026-08-30, before AWIA virtual staff existed, and none of the seven R4 smoke tests referenced AWIA. Rather than re-running that already-accepted, non-AWIA sequence, R4-S2 (staging deployment/data protection), R4-S4 (observability/audit review), and R4-S5 (private pilot cohort) were extended to verify AWIA virtual staff specifically operates correctly under Release 4's real staging controls - see ADR-072 in `DECISION_REGISTER.md`. **Phase D is CLOSED.**
...
**Status update (2026-09-06):** Phase E is DEFERRED to the end of this roadmap by explicit product-owner decision - see ADR-073 in `DECISION_REGISTER.md`. Investigation found vFirm has no real cryptographically-verifiable auth provider yet, which the Central Payment Hub's preferred authentication pattern (a real signed user JWT) depends on; building that first was judged out of order versus maturing the operating platform. Partial commercial decisions already captured for when Phase E resumes: payment provider = existing NHL Global Solution Stripe account via the Central Payment Hub (`registry/apps/vfirm` not yet created), `user_ref` = vFirm `firm_id`, entitlement keys `awia_seat_solostand` / `awia_seat_entgrow` / `awia_seat_corpoext` / `awia_seat_hireme_custom`. Next roadmap work is package-selection / hire-a-worker / business-operations for the two existing pilot firms (Amanah Formwork, NHL Global Solution) on top of the existing config-only seat-billing state machine, scope to be confirmed separately before its own "Proceed ... Bismillah" authorization.

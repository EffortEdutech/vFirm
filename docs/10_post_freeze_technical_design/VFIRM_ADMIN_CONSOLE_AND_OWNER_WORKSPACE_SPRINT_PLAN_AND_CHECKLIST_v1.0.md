...
Status: Accepted — all four phases complete (Phase 1: ADR-080; Phase 2: ADR-081; Phase 3: ADR-082; Phase 4: ADR-083)
...
- [x] Phase 3: Workdesk screen (5 tabs) built, Work/Approvals screens retired (ADR-082)
- [x] Phase 3: end-to-end Workdesk smoke test passing (`check:awia:work-assignment` + `check:awia:workdesk-archive` prove the full backend path; `check:web:navigation` proves the UI wiring)
- [x] Phase 4: all `check:web*` smoke tests re-confirmed passing (`check:web`, `check:web-api`, `check:web:multitenant`, `check:web:navigation` — product owner, PowerShell, ADR-083)
- [x] Phase 4: ADR(s) written, this document marked Accepted (ADR-083)

...
function renderSalesAccountsModule(store){
  ...
  const opportunities=store.sales_pipeline_records??[],proposals=store.proposals??[],expenses=store.expense_records??[],invoices=store.invoices??[],followUps=store.receivable_follow_ups??[],bindings=store.commercial_skill_bindings??[],quotationCases=store.quotation_cases??[],relationships=store.firm_client_relationships??[],documents=store.document_register_entries??[];

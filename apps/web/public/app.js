...
const workspaceCollections = [
  ...
  ["private_directory_governance_summary", "/marketplace/private-directory-governance-summary"],
  ["private_directory_intelligence_summary", "/marketplace/private-directory-intelligence-summary"],
  ["persons", "/persons"],
  ["actors", "/actors"],
...
const steps = [
  ...
  { key: "approval", title: "6. Approve Proposal", endpoint: "POST /proposals/approve", needs: ["tenant", "firm", "proposal"], run: () => request("/proposals/approve", { method: "POST", body: JSON.stringify({ tenant_id: state.tenant.id, firm_id: state.firm.id, proposal_id: state.proposal.id, actor: state.principal_actor }) }), apply: (data) => { state.approval = data.approval; state.proposal = data.proposal; } },
  { key: "project", title: "7. Accept Proposal / Open Project", endpoint: "POST /proposals/accept", needs: ["tenant", "firm", "proposal"], run: () => request("/proposals/accept", { method: "POST", body: JSON.stringify({ tenant_id: state.tenant.id, firm_id: state.firm.id, proposal_id: state.proposal.id, project_name: state.projectName, actor: state.principal_actor }) }), apply: (data) => { state.proposal = data.proposal; state.engagement = data.engagement; state.project = data.project; state.workPackage = data.workPackage; state.task = data.task; } },
  { key: "evidence", title: "8. Create Evidence Bundle", endpoint: "POST /evidence-bundles", needs: ["tenant", "firm", "project", "intake"], run: () => request("/evidence-bundles", { method: "POST", body: JSON.stringify({ tenant_id: state.tenant.id, firm_id: state.firm.id, project_id: state.project.id, subject_type: "Project", subject_id: state.project.id, input_refs: [state.intake.id], actor: state.principal_actor }) }), apply: (data) => { state.evidence = data; } },
  { key: "invoice", title: "9. Create Invoice", endpoint: "POST /invoices", needs: ["tenant", "firm", "relationship", "engagement", "project"], run: () => request("/invoices", { method: "POST", body: JSON.stringify({ tenant_id: state.tenant.id, firm_id: state.firm.id, relationship_id: state.relationship.id, engagement_id: state.engagement.id, project_id: state.project.id, currency: state.currency, line_items: [{ description: "Formwork Design Support - Preliminary Wall/Slab Package", amount: Number(state.finalPrice) }], actor: state.principal_actor }) }), apply: (data) => { state.invoice = data; } }
];
...
async function request(path, options = {}) {
  const res = await fetch(`${API_BASE}${path}`, { ...options, headers: { "content-type": "application/json", ...devAuthHeaders(), ...(options.headers ?? {}) } });
  const json = await res.json();
  if (!res.ok || !json.ok) throw new Error(json.error?.message ?? `Request failed: ${path}`);
  return json.data;
}

function formworkInputs() {
  return { project_name: state.projectName, site_location: "Kuala Lumpur", client_organization: state.clientName, client_contact_name: "Demo Contact", client_contact_email: "client@example.com", structure_type: "basement", formwork_element_type: "wall", height: 3.5, length_or_area: 120, concrete_grade: "C30", available_drawings: ["S-100"], deadline: new Date(Date.now() + 14 * 86400000).toISOString(), required_deliverables: ["preliminary_support_report"] };
}

function pickSummary(data = state) {
  return { tenant: data.tenant?.id, firm: data.firm?.id, principal_actor: data.principal_actor?.id, client: data.client?.id, relationship: data.relationship?.id, intake: data.intake?.id, proposal: data.proposal?.id, approval: data.approval?.id, engagement: data.engagement?.id, project: data.project?.id, work_package: data.workPackage?.id, task: data.task?.id, evidence: data.evidence?.id, invoice: data.invoice?.id };
}
...
function stepReady(step) { return step.needs.every((key) => state[key]?.id); }
function stepDone(step) { return Boolean(state[step.key]?.id); }
function escapeHtml(value) { return String(value ?? "").replace(/[&<>'"]/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" }[char])); }
function shortId(id) { return id ? String(id).replace(/^([a-z]+_)?/, "").slice(0, 8) : "-"; }
function money(value, currency = "MYR") { return `${currency} ${Number(value ?? 0).toLocaleString()}`; }
...
function commandErrorMessage(error) {
  ...
  if (/fetch|Failed to fetch|NetworkError/i.test(message)) return "The API is not reachable. Check that npm run dev is running and the API is listening on 3091.";
  return message;
}

async function runUiCommand({ label, button = null, form = null, success = "Done", action }) {
...
function workflowStage(store) {
  ...
  if ((store.proposals ?? []).some((proposal) => proposal.proposal_status === "APPROVED")) return "Proposal approved";
...
function healthTone(status) {
  if (["ONLINE", "ACTIVE", "INVOICE_READY", "DELIVERY_OPEN", "COMMERCIAL_ACTIVE", "FRONT_DOOR_ACTIVE", "FIRM_READY", "ACTIVE"].includes(status)) return "good";
  if (["WAITING_FOR_ACTIVITY", "SETUP_REQUIRED"].includes(status)) return "warn";
  return "bad";
}
...
function renderNextActions(store) {
  ...
  nextActions.querySelectorAll("button[data-action-view]").forEach((button) => button.addEventListener("click", () => switchView(button.dataset.actionView)));
}
...
function eventTone(type = "") {
  if (type.includes("approved") || type.includes("created") || type.includes("opened")) return "good";
  if (type.includes("denied") || type.includes("failed")) return "bad";
  return "neutral";
}

function renderTimeline(events) {
  return events.length ? `<ol class="timeline">${events.map((event) => `<li class="${eventTone(event.event_type)}"><span>${escapeHtml(event.event_type)}</span><p>${escapeHtml(event.payload_summary ?? event.aggregate_type ?? "Event recorded")}</p><code>${escapeHtml(event.occurred_at)}</code></li>`).join("")}</ol>` : `<p class="empty">No events yet. Run the workflow to create an audit trail.</p>`;
}

function formworkPack(store) {
  return (store.service_packs ?? []).find((pack) => pack.code === "VF-SP-001") ?? null;
}

function formworkSku(store) {
  return (store.service_skus ?? []).find((sku) => sku.code === "formwork_preliminary_wall_slab") ?? null;
}

function formworkEvidenceRequirements(store) {
  return formworkPack(store)?.configuration?.required_evidence ?? ["intake_summary", "basis_of_design", "preliminary_calculation_note", "qa_review_note"];
}

function intakeCompleteness(record) {
  const required = formworkPack(lastStore)?.configuration?.required_inputs ?? ["project_name", "site_location", "structure_type", "formwork_element_type", "height", "length_or_area", "concrete_grade", "available_drawings"];
  const provided = record?.provided_inputs ?? {};
  return required.map((key) => [key, Array.isArray(provided[key]) ? provided[key].length > 0 : Boolean(provided[key])]);
}
function renderWorkflow() {
  workflowSteps.innerHTML = steps.map((step) => {
    const done = stepDone(step); const ready = stepReady(step); const disabled = done || !ready;
    const status = done ? "Done" : ready ? "Ready" : `Needs ${step.needs.filter((key) => !state[key]?.id).join(", ")}`;
...
function renderSummary(store) {
  setOperatorMessage(`${workflowStage(store)} • ${store._dashboard_summary?.counts?.events ?? store.event_log?.length ?? 0} events recorded`, "ready");
...
function buildQuotationOperationsSummaryFallback(store) {
  ...
  if (extractionReview.length) exceptions.push({ key: "boq_extraction_review_pending", severity: "MEDIUM", count: extractionReview.length, detail: "BOQ extraction aid needs human review." });
  if (draftReview.length) exceptions.push({ key: "quotation_draft_review_pending", severity: "MEDIUM", count: draftReview.length, detail: "Quotation draft pack needs human review." });
  ...
  if (issuedWithoutReceivable.length) exceptions.push({ key: "issued_without_receivable_preparation", severity: "MEDIUM", count: issuedWithoutReceivable.length, detail: "Human-issued quotation has no receivable preparation." });
  if (receivableReview.length) exceptions.push({ key: "receivable_review_required", severity: "LOW", count: receivableReview.length, detail: "Receivable preparation is ready for review; no payment action exists." });
  return {
    status: exceptions.some((item) => item.severity === "HIGH") ? "OPERATOR_ATTENTION_REQUIRED" : exceptions.length ? "REVIEW_QUEUE_ACTIVE" : cases.length ? "QUOTATION_PIPELINE_CLEAR" : "NO_QUOTATION_ACTIVITY",
    counts: { quotation_cases: cases.length, boq_extraction_aids: aids.length, quotation_draft_packs: drafts.length, quotation_issue_records: issues.length, receivable_preparations: receivables.length, pending_human_reviews: extractionReview.length + draftReview.length + issueReady.length + receivableReview.length, issue_ready: issueReady.length, issued_without_receivable: issuedWithoutReceivable.length, correspondence_review: correspondenceReview.length },
    exceptions,
    approvals: [].concat(extractionReview.map((item) => ({ type: "boq_extraction_aid_review", id: item.id, status: item.extraction_status })), draftReview.map((item) => ({ type: "quotation_draft_pack_review", id: item.id, status: item.draft_status })), issueReady.map((item) => ({ type: "controlled_quotation_issue", id: item.id, status: item.draft_status })), receivableReview.map((item) => ({ type: "receivable_preparation_review", id: item.id, status: item.receivable_status }))),
    boundaries: ["advisory_boq_extraction_only", "human_controlled_quotation_issue", "no_live_payment_movement", "tenant_scoped_audit_export"]
  };
}

function renderQuotationOperationsSummary(store, options = {}) {
  const summary = store.quotation_operations_summary ?? buildQuotationOperationsSummaryFallback(store);
  const counts = summary.counts ?? {};
  const exceptions = summary.exceptions ?? [];
  const approvals = summary.approvals ?? [];
  const exceptionList = exceptions.slice(0, 6).map((item) => '<li><strong>' + escapeHtml(item.severity ?? "REVIEW") + '</strong><span>' + escapeHtml(item.detail ?? item.key) + (item.count ? ' (' + escapeHtml(item.count) + ')' : '') + '</span></li>').join("");
  const approvalList = approvals.slice(0, 6).map((item) => '<li><strong>' + escapeHtml(humanStatus(item.type)) + '</strong><span>' + escapeHtml(item.status ?? shortId(item.id)) + '</span></li>').join("");
  const boundaryList = (summary.boundaries ?? []).map((item) => '<span>' + escapeHtml(humanStatus(item)) + '</span>').join("");
  return '<section class="panel nhl-q5-quotation-operations"><div class="panel-heading"><h2>' + escapeHtml(options.title ?? "NHL-Q5 Quotation Operations") + '</h2><p>Active workspace quotation pipeline, exception handling, review queue, receivable readiness, and audit posture.</p></div>' + renderRelatedContext("Quotation readiness", [["Status", humanStatus(summary.status)], ["Cases", counts.quotation_cases ?? 0], ["BOQ aids", counts.boq_extraction_aids ?? 0], ["Draft packs", counts.quotation_draft_packs ?? 0], ["Issue ready", counts.issue_ready ?? 0], ["Issued", counts.quotation_issue_records ?? 0], ["Receivable prep", counts.receivable_preparations ?? 0], ["Pending human reviews", counts.pending_human_reviews ?? 0]]) + '<div class="grid two"><div><h3>Exceptions and next actions</h3><ul class="plain-list">' + (exceptionList || '<li><strong>Clear</strong><span>No quotation exception surfaced.</span></li>') + '</ul></div><div><h3>Human review queue</h3><ul class="plain-list">' + (approvalList || '<li><strong>Clear</strong><span>No quotation review item waiting.</span></li>') + '</ul></div></div><div class="checklist warning">' + boundaryList + '</div></section>';
}
...
function renderLatestActivity(store) {
  const events = store._dashboard_summary?.latest_activity ?? [...(store.event_log ?? [])].slice(-6).reverse();
  latestActivity.innerHTML = events.length ? events.map((event) => `<article class="activity-item"><strong>${escapeHtml(event.event_type)}</strong><span>${escapeHtml(event.payload_summary)}</span><code>${escapeHtml(event.occurred_at)}</code></article>`).join("") : `<p class="empty">No events yet. Run the workflow to create activity.</p>`;
}

function relationName(store, relationshipId) {
  const rel = store.firm_client_relationships?.find((item) => item.id === relationshipId);
...
function personName(store, personId) {
  ...
  const composed = [person?.given_name, person?.family_name].filter(Boolean).join(" ");
  return person?.display_name ?? person?.full_name ?? person?.name ?? (composed || shortId(personId));
}

function renderRecordView({ target, title, description, records, columns, empty }) {
  ...
  host.querySelectorAll("button[data-detail-index]").forEach((button) => button.addEventListener("click", () => {
    const record = records[Number(button.dataset.detailIndex)];
    host.querySelector(`#detail-${target.slice(1)}`).innerHTML = renderHumanDetail(title, [["Status", record.status ?? record.decision ?? record.proposal_status ?? record.project_state], ["Tenant", shortId(record.tenant_id)], ["Firm", shortId(record.firm_id)], ["Record ID", shortId(record.id)]], record);
  }));
}
...
function subscribedModuleCodes(store) {
  return new Set(activeWorkspaceContract(store).modules.map((module) => module.module_code));
}
...
function workerTemplateCodesForContract(contract) {
  return new Set((contract.profile.worker_templates ?? []).concat(contract.modules.map((module) => module.worker_template_code).filter(Boolean)));
}

function defaultWorkerNameForTemplate(templateCode, contract) {
  const prefix = contract.profile.firm_type === "ORGANIZATION_SUPPORT" ? "Organization Support" : contract.profile.firm_type === "FORMWORK_ENGINEERING" ? "Formwork" : "Virtual Firm";
  const names = {
    ...
    "technical-drawing-assistant": contract.profile.firm_type === "ORGANIZATION_SUPPORT" ? "NHL Technical Writing and EDCS Document AI Worker" : `${prefix} Technical Drawing AI Worker`,
...
function defaultServiceHint(contract) {
  return contract.serviceLines[0]?.service_name ?? workspaceServiceSummary(contract);
}

function defaultOutputRef(contract) {
  return contract.profile.firm_type === "ORGANIZATION_SUPPORT" ? "ai://outputs/organization-support-draft" : "ai://outputs/formwork-intake-summary";
}
...
function quotationCaseRows(store) {
  const cases = store.quotation_cases ?? [];
  if (!cases.length) return `<p class="empty">No quotation cases yet for ${escapeHtml(activeWorkspaceContract(store).firm?.name ?? "the active firm")}.</p>`;
...
function boqExtractionAidRows(store) {
  const aids = store.boq_extraction_aids ?? [];
  if (!aids.length) return `<p class="empty">No BOQ extraction aids prepared yet.</p>`;
...
function quotationDraftPackRows(store) {
  ...
  if (!packs.length) return `<p class="empty">No quotation draft packs assembled yet.</p>`;
...
function quotationIssueRows(store) {
  ...
  if (!records.length) return '<p class="empty">No controlled quotation issues recorded yet.</p>';
  return '<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Issue</th><th>Status</th><th>Issued to</th><th>Evidence</th><th>Payment</th></tr></thead><tbody>' + records.map((item) => '<tr><td>' + escapeHtml(shortId(item.id)) + '<br/><small>' + escapeHtml(shortId(item.quotation_draft_pack_id)) + '</small></td><td><span class="pill">' + escapeHtml(item.issue_status) + '</span></td><td>' + escapeHtml(item.issued_to ?? "Client") + '</td><td>' + escapeHtml(item.submitted_evidence_ref ?? "Not recorded") + '</td><td>' + (item.payment_action_taken ? "Payment action" : "No payment action") + '</td></tr>').join("") + '</tbody></table></div>';
}

function quotationReceivablePreparationRows(store) {
  const records = store.quotation_receivable_preparations ?? [];
  if (!records.length) return '<p class="empty">No receivable preparations recorded yet.</p>';
  return '<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Receivable prep</th><th>Status</th><th>Amount summary</th><th>Invoice draft</th><th>Boundary</th></tr></thead><tbody>' + records.map((item) => '<tr><td>' + escapeHtml(shortId(item.id)) + '<br/><small>' + escapeHtml(shortId(item.quotation_issue_record_id)) + '</small></td><td><span class="pill">' + escapeHtml(item.receivable_status) + '</span></td><td>' + escapeHtml(item.amount_summary ?? "Review required") + '</td><td>' + escapeHtml(item.invoice_draft_ref ?? "Not prepared") + '</td><td>' + escapeHtml(item.payment_boundary ?? "NO_LIVE_PAYMENT_MOVEMENT") + '</td></tr>').join("") + '</tbody></table></div>';
}

function renderFrontDeskModule(store) {
  const enquiries = store.front_desk_enquiries ?? [], drafts = store.client_communication_drafts ?? [];
  const contract = activeWorkspaceContract(store);
  const tenant = contract.tenant, firm = contract.firm, actor = contract.principal;
  ...
  document.querySelector("#enquiryForm")?.addEventListener("submit", async (event) => { event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({ label:"Capture enquiry", form:event.currentTarget, success:"Enquiry captured", action:async()=>{ const data=await request("/front-desk/enquiries",{method:"POST",body:JSON.stringify({tenant_id:tenant.id,firm_id:firm.id,contact_name:fd.get("contact_name"),organization_name:fd.get("organization_name"),contact_email:fd.get("contact_email"),enquiry_summary:fd.get("enquiry_summary"),requested_service_hint:fd.get("requested_service_hint") || defaultServiceHint(contract),actor:act})}); await refresh(); switchView("front-desk"); return data; }}); });
  document.querySelector("#qualifyEnquiryForm")?.addEventListener("submit", async (event) => { event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({ label:"Qualify enquiry", form:event.currentTarget, success:"Enquiry qualified", action:async()=>{ const data=await request("/front-desk/enquiries/qualify",{method:"POST",body:JSON.stringify({tenant_id:tenant.id,firm_id:firm.id,enquiry_id:fd.get("enquiry_id"),decision:"QUALIFIED",consent_or_legal_basis_ref:fd.get("consent_or_legal_basis_ref"),conflict_check_status:"CLEARED",conflict_check_ref:fd.get("conflict_check_ref"),actor:act})}); await refresh(); switchView("front-desk"); return data; }}); });
  document.querySelector("#draftCommunicationForm")?.addEventListener("submit", async (event) => { event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({ label:"Draft acknowledgement", form:event.currentTarget, success:"Draft saved for review", action:async()=>{ const data=await request("/front-desk/communication-drafts",{method:"POST",body:JSON.stringify({tenant_id:tenant.id,firm_id:firm.id,enquiry_id:fd.get("enquiry_id"),message_body:fd.get("message_body"),actor:act})}); await refresh(); switchView("front-desk"); return data; }}); });
  document.querySelector("#handoffEnquiryForm")?.addEventListener("submit", async (event) => { event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({ label:"Handoff to intake", form:event.currentTarget, success:"Enquiry handed off with intake controls", action:async()=>{ const data=await request("/front-desk/enquiries/handoff",{method:"POST",body:JSON.stringify({tenant_id:tenant.id,firm_id:firm.id,enquiry_id:fd.get("enquiry_id"),provided_inputs:{},actor:act})}); await refresh(); switchView("front-desk"); return data; }}); });
}
function renderAdministrationModule(store) {
  ...
  const docs=store.document_register_entries??[], revisions=store.document_revision_records??[], deadlines=store.administrative_deadlines??[], correspondence=store.correspondence_records??[], transmittals=store.transmittal_drafts??[],bindings=store.administration_skill_bindings??[];
  const docOptions=docs.map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(x.document_number)} — ${escapeHtml(x.title)}</option>`).join("");
  const revOptions=revisions.filter(x=>x.status==="CURRENT").map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(docs.find(d=>d.id===x.document_register_entry_id)?.document_number??"-")} Rev ${escapeHtml(x.revision)}</option>`).join("");
  const deadlineOptions=deadlines.filter(x=>x.status==="OPEN").map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(x.title)}</option>`).join("");
  document.querySelector("#administrationView").innerHTML=`<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Administration Clerk</h2><p>Bounded skills, schemas, permissions, supervisor, and prohibited actions.</p></div><form id="adminBindingForm"><label>Role skill ref<input name="role_skill_ref" value="skills://roles/administration-clerk/v1" /></label><label>Worker skill ref<input name="worker_skill_ref" value="skills://workers/document-controller/v1" /></label><button type="submit" ${tenant&&firm&&!bindings.length?"":"disabled"}>Activate Binding</button></form><form id="correspondenceForm"><label>Correspondent<input name="correspondent" value="Client Project Manager" /></label><label>Subject<input name="subject" value="Incoming drawing package" /></label><button type="submit" ${tenant&&firm?"":"disabled"}>Register Correspondence</button></form></section><section class="panel compact-form"><div class="panel-heading"><h2>Document Register</h2><p>Document numbers and revisions are deterministic and immutable.</p></div><form id="documentRegisterForm"><label>Document no.<input name="document_number" value="FW-DWG-001" /></label><label>Title<input name="title" value="Wall Formwork General Arrangement" /></label><label>Revision<input name="revision" value="A" /></label><label>Storage ref<input name="storage_ref" value="local://documents/FW-DWG-001-A.pdf" /></label><button type="submit" ${tenant&&firm?"":"disabled"}>Register Document</button></form><form id="documentRevisionForm"><label>Document<select name="document_register_entry_id">${docOptions}</select></label><label>New revision<input name="revision" value="B" /></label><label>Storage ref<input name="storage_ref" value="local://documents/FW-DWG-001-B.pdf" /></label><button type="submit" ${docs.length?"":"disabled"}>Add Revision</button></form></section></section><section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Deadlines</h2><p>Open follow-ups remain visible until explicitly completed.</p></div><form id="deadlineForm"><label>Title<input name="title" value="Review incoming drawing package" /></label><label>Due<input name="due_at" type="datetime-local" /></label><button type="submit" ${tenant&&firm?"":"disabled"}>Create Deadline</button></form><form id="deadlineCompleteForm"><label>Open deadline<select name="deadline_id">${deadlineOptions}</select></label><button type="submit" ${deadlineOptions?"":"disabled"}>Complete Deadline</button></form></section><section class="panel compact-form"><div class="panel-heading"><h2>Transmittal Preparation</h2><p>Draft only. Principal approval and issue are intentionally unavailable.</p></div><form id="transmittalForm"><label>Recipient<input name="recipient" value="Client Project Manager" /></label><label>Subject<input name="subject" value="Drawing package for review" /></label><label>Current revision<select name="document_revision_ref">${revOptions}</select></label><label>Message<input name="message_body" value="Please find the listed document prepared for review." /></label><button type="submit" ${revOptions?"":"disabled"}>Prepare Review Draft</button></form></section></section><section class="panel"><div class="panel-heading"><h2>Administration Register</h2><p>${correspondence.length} correspondence · ${docs.length} documents · ${deadlines.filter(x=>x.status==="OPEN").length} open deadlines · ${transmittals.length} review drafts.</p></div>${docs.length?`<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Document</th><th>Current revision</th><th>Status</th></tr></thead><tbody>${docs.map(d=>{const r=revisions.find(x=>x.id===d.current_revision_id);return `<tr><td>${escapeHtml(d.document_number)}<br/><small>${escapeHtml(d.title)}</small></td><td>${escapeHtml(r?.revision??"-")}</td><td><span class="pill">${escapeHtml(d.status)}</span></td></tr>`}).join("")}</tbody></table></div>`:'<p class="empty">No documents registered.</p>'}</section>`;
  const run=(selector,label,path,build,success)=>document.querySelector(selector)?.addEventListener("submit",async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);await runUiCommand({label,form:e.currentTarget,success,action:async()=>{const data=await request(path,{method:"POST",body:JSON.stringify({...build(fd),tenant_id:tenant.id,firm_id:firm.id,actor:auth})});await refresh();switchView("administration");return data;}})});
  run("#adminBindingForm","Activate Administration binding","/administration/skill-bindings",fd=>({role_skill_ref:fd.get("role_skill_ref"),worker_skill_ref:fd.get("worker_skill_ref")}),"Administration Clerk binding activated");
  run("#correspondenceForm","Register correspondence","/administration/correspondence",fd=>({correspondent:fd.get("correspondent"),subject:fd.get("subject"),direction:"INCOMING",channel:"EMAIL"}),"Correspondence registered");
  run("#documentRegisterForm","Register document","/administration/documents",fd=>({document_number:fd.get("document_number"),title:fd.get("title"),document_type:"DRAWING",revision:fd.get("revision"),storage_ref:fd.get("storage_ref"),content_hash:`manual:${fd.get("document_number")}:${fd.get("revision")}`}),"Document registered");
  run("#documentRevisionForm","Add revision","/administration/document-revisions",fd=>({document_register_entry_id:fd.get("document_register_entry_id"),revision:fd.get("revision"),storage_ref:fd.get("storage_ref"),content_hash:`manual:${fd.get("document_register_entry_id")}:${fd.get("revision")}`}),"Revision registered");
  run("#deadlineForm","Create deadline","/administration/deadlines",fd=>({title:fd.get("title"),due_at:fd.get("due_at")||new Date(Date.now()+86400000).toISOString()}),"Deadline created");
  run("#deadlineCompleteForm","Complete deadline","/administration/deadlines/complete",fd=>({deadline_id:fd.get("deadline_id")}),"Deadline completed");
  run("#transmittalForm","Prepare transmittal","/administration/transmittal-drafts",fd=>({recipient:fd.get("recipient"),subject:fd.get("subject"),document_revision_refs:[fd.get("document_revision_ref")],message_body:fd.get("message_body")}),"Transmittal draft saved for principal review");
}
...
function renderClientModule(store) {
  ...
  const rows = records.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Name</th><th>Type</th><th>Status</th><th>Relationship</th><th>Detail</th></tr></thead><tbody>${records.map((record, index) => {
    const relationship = store.firm_client_relationships?.find((item) => item.client_id === record.id);
  ...
  }).join("")}</tbody></table></div><pre class="output detail-output" id="clientDetailBox">Select a client.</pre>` : `<p class="empty">No clients yet. Create a tenant and firm first, then add a client here.</p>`;
  ...
  document.querySelector("#clientCreateForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    await runUiCommand({ label: "Create client", form: event.currentTarget, success: "Client created", action: async () => {
      const data = await request("/clients", { method: "POST", body: JSON.stringify({ tenant_id: latestTenant.id, firm_id: latestFirm.id, name: formData.get("name"), client_type: formData.get("client_type"), actor: principalActor ?? systemActorForBrowser(latestTenant.id, latestFirm.id) }) });
  ...
  document.querySelectorAll("button[data-client-detail]").forEach((button) => button.addEventListener("click", () => {
    const record = records[Number(button.dataset.clientDetail)];
    const relationship = store.firm_client_relationships?.find((item) => item.client_id === record.id);
    document.querySelector("#clientDetailBox").innerHTML = renderHumanDetail(record.name, [["Client type", record.client_type], ["Status", record.status], ["Relationship", shortId(relationship?.id)], ["Tenant", shortId(record.tenant_id)], ["Firm", shortId(record.firm_id)], ["Created", record.created_at]], { client: record, relationship }) + renderRelatedContext("Related workflow", [["Intake sessions", countBy(store.leads, (lead) => lead.relationship_id === relationship?.id)], ["Proposals", countBy(store.proposals, (proposal) => proposal.relationship_id === relationship?.id)], ["Projects", countBy(store.projects, (project) => project.relationship_id === relationship?.id)], ["Invoices", countBy(store.invoices, (invoice) => invoice.relationship_id === relationship?.id)]]);
  }));
}
...
function renderIntakeModule(store) {
  ...
  document.querySelector("#intakeCreateForm")?.addEventListener("submit", async (event) => {
    ...
    await runUiCommand({ label: "Create intake", form: event.currentTarget, success: "Intake session created", action: async () => {
      const data = await request("/intake-sessions", { method: "POST", body: JSON.stringify({ tenant_id: latestTenant.id, firm_id: latestFirm.id, relationship_id: relationship.id, actor: principalActor ?? systemActorForBrowser(latestTenant.id, latestFirm.id), provided_inputs }) });
  ...
  document.querySelectorAll("button[data-intake-detail]").forEach((button) => button.addEventListener("click", () => {
    const record = records[Number(button.dataset.intakeDetail)];
    const lead = store.leads?.find((item) => item.id === record.lead_id);
    const completeness = intakeCompleteness(record); document.querySelector("#intakeDetailBox").innerHTML = renderHumanDetail(record.provided_inputs?.project_name ?? "Formwork intake", [["Status", record.intake_status], ["Missing items", record.missing_information_items?.length ?? 0], ["Service", shortId(record.service_id)], ["Lead", shortId(record.lead_id)], ["Site", record.provided_inputs?.site_location], ["Element", record.provided_inputs?.formwork_element_type]], { intake: record, lead }) + renderRelatedContext("Intake completeness", completeness.map(([key, ok]) => [key, ok ? "Complete" : "Missing"]));
  }));
}

function systemActorForBrowser(tenant_id, firm_id) {
  return { actor_id: "system", actor_type: "SYSTEM", tenant_id, firm_id, display_name: "vFirm System" };
}
function renderProposalModule(store) {
  const latestFirm = latestRecord(store, "firms");
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const completeIntakes = (store.intake_sessions ?? []).filter((item) => item.intake_status === "COMPLETE");
  const proposals = store.proposals ?? [];
  const intakeOptions = completeIntakes.map((intake) => `<option value="${escapeHtml(intake.id)}">${escapeHtml(shortId(intake.id))} - ${escapeHtml(intake.provided_inputs?.project_name ?? "Formwork intake")}</option>`).join("");
  const proposalOptions = proposals.map((proposal) => `<option value="${escapeHtml(proposal.id)}">${escapeHtml(shortId(proposal.id))} - ${escapeHtml(proposal.proposal_status)} - ${escapeHtml(relationName(store, proposal.relationship_id))}</option>`).join("");
  const approvedOptions = proposals.filter((proposal) => ["APPROVED", "SENT"].includes(proposal.proposal_status)).map((proposal) => `<option value="${escapeHtml(proposal.id)}">${escapeHtml(shortId(proposal.id))} - ${escapeHtml(relationName(store, proposal.relationship_id))}</option>`).join("");
  const canCreateProposal = Boolean(completeIntakes.length && latestTenant && latestFirm);
  const canApproveProposal = Boolean(proposalOptions);
  const canAcceptProposal = Boolean(approvedOptions);
  const rows = proposals.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Client</th><th>Status</th><th>Scope</th><th>Price Ref</th><th>Detail</th></tr></thead><tbody>${proposals.map((record, index) => `<tr><td>${escapeHtml(relationName(store, record.relationship_id))}</td><td><span class="pill">${escapeHtml(record.proposal_status)}</span></td><td>${escapeHtml(record.scope_summary)}</td><td>${escapeHtml(shortId(record.price_build_up_id))}</td><td><button class="secondary small" data-proposal-detail="${index}">View</button></td></tr>`).join("")}</tbody></table></div><pre class="output detail-output" id="proposalDetailBox">Select a proposal.</pre>` : `<p class="empty">No proposals yet. Complete an intake first, then create a proposal here.</p>`;

  document.querySelector("#proposalsView").innerHTML = `<section class="grid two"><form id="proposalCreateForm" class="panel compact-form"><div class="panel-heading"><h2>Create Proposal</h2><p>Create a scope and price build-up from a completed intake.</p></div><label>Completed intake<select name="intake_session_id" required>${intakeOptions}</select></label><label>Scope summary<input name="scope_summary" value="Preliminary formwork design support package" required /></label><label>Final price<input name="final_price" type="number" value="2500" min="0" /></label><button type="submit" ${canCreateProposal ? "" : "disabled"}>Create Proposal</button>${disabledHint(canCreateProposal, "Complete an intake before creating a proposal.")}<p class="form-note">Endpoint: POST /proposals</p></form><section class="panel compact-form"><div class="panel-heading"><h2>Proposal Actions</h2><p>Approve proposal, then accept it to open a project.</p></div><form id="proposalApproveForm"><label>Proposal<select name="proposal_id" required>${proposalOptions}</select></label><button type="submit" ${canApproveProposal ? "" : "disabled"}>Approve Proposal</button>${disabledHint(canApproveProposal, "Create a proposal before approval.")}<p class="form-note">Endpoint: POST /proposals/approve</p></form><hr class="soft-divider" /><form id="proposalAcceptForm"><label>Approved proposal<select name="proposal_id" required>${approvedOptions}</select></label><label>Project name<input name="project_name" value="Basement Wall Formwork Package" required /></label><button type="submit" ${canAcceptProposal ? "" : "disabled"}>Accept / Open Project</button>${disabledHint(canAcceptProposal, "Approve a proposal before accepting it.")}<p class="form-note">Endpoint: POST /proposals/accept</p></form></section></section><section class="panel"><div class="panel-heading"><h2>Proposals</h2><p>Proposal and price records.</p></div>${rows}</section>`;

  document.querySelector("#proposalCreateForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    const intake = completeIntakes.find((item) => item.id === formData.get("intake_session_id"));
    const lead = store.leads?.find((item) => item.id === intake?.lead_id);
    await runUiCommand({ label: "Create proposal", form: event.currentTarget, success: "Proposal created", action: async () => {
      const data = await request("/proposals", { method: "POST", body: JSON.stringify({ tenant_id: latestTenant.id, firm_id: latestFirm.id, relationship_id: lead.relationship_id, intake_session_id: intake.id, scope_summary: formData.get("scope_summary"), final_price: Number(formData.get("final_price") || 0), actor: principalActor ?? systemActorForBrowser(latestTenant.id, latestFirm.id) }) });
  ...
  document.querySelector("#proposalApproveForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const proposal_id = new FormData(event.currentTarget).get("proposal_id");
    await runUiCommand({ label: "Approve proposal", form: event.currentTarget, success: "Proposal approved", action: async () => {
      const data = await request("/proposals/approve", { method: "POST", body: JSON.stringify({ tenant_id: latestTenant.id, firm_id: latestFirm.id, proposal_id, actor: principalActor ?? systemActorForBrowser(latestTenant.id, latestFirm.id) }) });
  ...
  document.querySelector("#proposalAcceptForm")?.addEventListener("submit", async (event) => {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    await runUiCommand({ label: "Accept proposal", form: event.currentTarget, success: "Project opened", action: async () => {
      const data = await request("/proposals/accept", { method: "POST", body: JSON.stringify({ tenant_id: latestTenant.id, firm_id: latestFirm.id, proposal_id: formData.get("proposal_id"), project_name: formData.get("project_name"), actor: principalActor ?? systemActorForBrowser(latestTenant.id, latestFirm.id) }) });
      state.proposal = data.proposal;
  ...
  document.querySelectorAll("button[data-proposal-detail]").forEach((button) => button.addEventListener("click", () => {
    const proposal = proposals[Number(button.dataset.proposalDetail)];
    const price = store.price_build_ups?.find((item) => item.id === proposal.price_build_up_id);
    const approvals = store.approvals?.filter((item) => item.subject_type === "Proposal" && item.subject_id === proposal.id) ?? [];
    document.querySelector("#proposalDetailBox").innerHTML = renderHumanDetail(proposal.scope_summary, [["Status", proposal.proposal_status], ["Client", relationName(store, proposal.relationship_id)], ["Price", money(price?.final_price, "MYR")], ["Approval ref", shortId(proposal.commercial_approval_id)], ["Valid until", proposal.valid_until], ["Version", proposal.version]], { proposal, price, approvals }) + renderRelatedContext("Commercial context", [["Approval decisions", approvals.length], ["Engagement", shortId((store.engagements ?? []).find((engagement) => engagement.proposal_id === proposal.id)?.id)], ["Project", shortId((store.projects ?? []).find((project) => project.relationship_id === proposal.relationship_id)?.id)], ["Policy decisions", countBy(store.policy_decisions, (decision) => decision.resource_id === proposal.id)]]);
  }));
}
...
function renderProjectModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const projectOptions = projects.map((project) => `<option value="${escapeHtml(project.id)}">${escapeHtml(project.project_name)} - ${escapeHtml(shortId(project.id))}</option>`).join("");
  const latestProject = latestRecord(store, "projects");
  const latestTask = latestProject ? [...(store.tasks ?? [])].reverse().find((task) => task.project_id === latestProject.id) : null;
  const latestEvidence = latestProject ? [...(store.evidence_bundles ?? [])].reverse().find((evidence) => evidence.project_id === latestProject.id) : null;
  const latestVersion = latestProject ? [...(store.document_versions ?? [])].reverse().find((version) => (store.documents ?? []).some((doc) => doc.id === version.document_id && doc.project_id === latestProject.id)) : null;
  const latestDeliverableApproval = latestVersion ? [...(store.approvals ?? [])].reverse().find((approval) => approval.subject_type === "DocumentVersion" && approval.subject_id === latestVersion.id) : null;
  const canRunProjectActions = Boolean(projects.length);
  const latestWorkPackage = latestProject ? [...(store.work_packages ?? [])].reverse().find((item) => item.project_id === latestProject.id) : null;
  const requiredEvidence = latestWorkPackage?.required_evidence ?? formworkEvidenceRequirements(store);
  const rows = projects.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Project</th><th>State</th><th>Risk</th><th>Tasks</th><th>Evidence</th><th>Docs</th><th>Detail</th></tr></thead><tbody>${projects.map((project, index) => `<tr><td>${escapeHtml(project.project_name)}</td><td><span class="pill">${escapeHtml(project.project_state)}</span></td><td>${escapeHtml(project.risk_class)}</td><td>${(store.tasks ?? []).filter((item)=>item.project_id===project.id).length}</td><td>${(store.evidence_bundles ?? []).filter((item)=>item.project_id===project.id).length}</td><td>${(store.documents ?? []).filter((item)=>item.project_id===project.id).length}</td><td><button class="secondary small" data-project-detail="${index}">View</button></td></tr>`).join("")}</tbody></table></div><pre class="output detail-output" id="projectDetailBox">Select a project.</pre>` : `<p class="empty">No projects yet. Accept an approved proposal first.</p>`;
  document.querySelector("#projectsView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Delivery Engine Actions</h2><p>Move the project through task, evidence, review, issue, and invoice gates.</p></div><form id="taskStartForm"><label>Project<select name="project_id" required>${projectOptions}</select></label><button type="submit" ${latestTask && ["CREATED","READY"].includes(latestTask.state) ? "" : "disabled"}>Start Current Task</button>${disabledHint(Boolean(latestTask && ["CREATED","READY"].includes(latestTask.state)), "Open a project with a created task before starting.")}<p class="form-note">Endpoint: POST /tasks/start</p></form><form id="taskCompleteForm"><button type="submit" ${latestTask && latestTask.state !== "COMPLETE" ? "" : "disabled"}>Complete Current Task</button>${disabledHint(Boolean(latestTask && latestTask.state !== "COMPLETE"), "Start or create a task before completion.")}<p class="form-note">Endpoint: POST /tasks/complete</p></form><hr class="soft-divider" /><form id="projectEvidenceForm"><label>Project<select name="project_id" required>${projectOptions}</select></label><button type="submit" ${canRunProjectActions ? "" : "disabled"}>Create Complete Evidence Bundle</button>${disabledHint(canRunProjectActions, "Open a project before creating evidence.")}<p class="form-note">Endpoint: POST /evidence-bundles</p></form><form id="deliverableDraftForm"><button type="submit" ${canRunProjectActions ? "" : "disabled"}>Create Deliverable Draft</button><p class="form-note">Endpoint: POST /deliverables/draft</p></form><form id="deliverableReviewForm"><button type="submit" ${latestEvidence && latestVersion ? "" : "disabled"}>Review / Approve Deliverable</button>${disabledHint(Boolean(latestEvidence && latestVersion), "Create evidence and a deliverable draft first.")}<p class="form-note">Endpoint: POST /deliverables/review</p></form><form id="deliverableIssueForm"><button type="submit" ${latestDeliverableApproval && latestVersion ? "" : "disabled"}>Issue Deliverable</button>${disabledHint(Boolean(latestDeliverableApproval && latestVersion), "Professional review approval is required before issue.")}<p class="form-note">Endpoint: POST /deliverables/issue</p></form><hr class="soft-divider" /><form id="projectInvoiceForm"><label>Amount<input name="amount" type="number" value="2500" min="0" /></label><button type="submit" ${canRunProjectActions ? "" : "disabled"}>Create Invoice</button>${disabledHint(canRunProjectActions, "Open a project before creating an invoice.")}<p class="form-note">Endpoint: POST /invoices</p></form></section><section class="panel"><div class="panel-heading"><h2>Project Detail</h2><p>Project, task, evidence, deliverable version, approval, and invoice context.</p></div><pre class="output detail-output" id="projectContextBox">Select a project or run an action.</pre></section></section><section class="panel"><div class="panel-heading"><h2>Projects</h2><p>Projects opened from accepted proposals.</p></div>${rows}</section>`;
  function context(project){ const docs=(store.documents??[]).filter((x)=>x.project_id===project.id); const versions=(store.document_versions??[]).filter((x)=>docs.some((doc)=>doc.id===x.document_id)); return { project, engagement:(store.engagements??[]).find((x)=>x.id===project.engagement_id), work_packages:(store.work_packages??[]).filter((x)=>x.project_id===project.id), tasks:(store.tasks??[]).filter((x)=>x.project_id===project.id), evidence_bundles:(store.evidence_bundles??[]).filter((x)=>x.project_id===project.id), documents:docs, document_versions:versions, invoices:(store.invoices??[]).filter((x)=>x.project_id===project.id) }; }
  function renderProjectContext(project){ const ctx=context(project); return renderHumanDetail(project.project_name, [["State", project.project_state], ["Risk", project.risk_class], ["Engagement", shortId(project.engagement_id)], ["Tasks", ctx.tasks.length], ["Evidence", ctx.evidence_bundles.length], ["Documents", ctx.documents.length], ["Invoices", ctx.invoices.length]], ctx) + renderRelatedContext("Stage 5 delivery gates", [["Task state", ctx.tasks[0]?.state ?? "No task"], ["Evidence status", ctx.evidence_bundles.at(-1)?.status ?? "No evidence"], ["Document version", ctx.document_versions.at(-1)?.status ?? "No draft"], ["Review approval", (store.approvals ?? []).find((approval)=>approval.subject_id===ctx.document_versions.at(-1)?.id)?.decision ?? "Not approved"], ["Required evidence", requiredEvidence.join(", ")], ["Invoice status", ctx.invoices[0]?.status ?? "Not created"]]); }
  document.querySelectorAll("button[data-project-detail]").forEach((button)=>button.addEventListener("click",()=>{ const project=projects[Number(button.dataset.projectDetail)]; const html=renderProjectContext(project); document.querySelector("#projectDetailBox").innerHTML=html; document.querySelector("#projectContextBox").innerHTML=html; }));
  document.querySelector("#taskStartForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); await runUiCommand({ label:"Start task", form:event.currentTarget, success:"Task started", action:async()=>{ const data=await request("/tasks/start",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,task_id:latestTask.id,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#taskCompleteForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); await runUiCommand({ label:"Complete task", form:event.currentTarget, success:"Task completed", action:async()=>{ const data=await request("/tasks/complete",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,task_id:latestTask.id,output_ref:"formwork_intake_completeness",actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#projectEvidenceForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const project=projects.find((x)=>x.id===new FormData(event.currentTarget).get("project_id")); await runUiCommand({ label:"Create complete evidence bundle", form:event.currentTarget, success:"Evidence bundle created", action:async()=>{ const data=await request("/evidence-bundles",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,project_id:project.id,subject_type:"Project",subject_id:project.id,input_refs:requiredEvidence,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); state.evidence=data; await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#deliverableDraftForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); await runUiCommand({ label:"Create deliverable draft", form:event.currentTarget, success:"Deliverable draft created", action:async()=>{ const data=await request("/deliverables/draft",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,project_id:latestProject.id,relationship_id:latestProject.relationship_id,title:`${latestProject.project_name} Preliminary Report`,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#deliverableReviewForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); await runUiCommand({ label:"Review deliverable", form:event.currentTarget, success:"Deliverable approved", action:async()=>{ const data=await request("/deliverables/review",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,project_id:latestProject.id,document_version_id:latestVersion.id,evidence_bundle_id:latestEvidence.id,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#deliverableIssueForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); await runUiCommand({ label:"Issue deliverable", form:event.currentTarget, success:"Deliverable issued", action:async()=>{ const data=await request("/deliverables/issue",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,project_id:latestProject.id,document_version_id:latestVersion.id,evidence_bundle_id:latestEvidence.id,approval_id:latestDeliverableApproval.id,subject_version_or_hash:latestVersion.hash,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("projects"); return data; }}); });
  document.querySelector("#projectInvoiceForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({ label:"Create invoice", form:event.currentTarget, success:"Invoice created", action:async()=>{ const data=await request("/invoices",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,relationship_id:latestProject.relationship_id,engagement_id:latestProject.engagement_id,project_id:latestProject.id,currency:"MYR",line_items:[{description:"Formwork Design Support - Preliminary Wall/Slab Package",amount:Number(fd.get("amount")||0)}],actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); state.invoice=data; await refresh(); switchView("invoices"); return data; }}); });
}
function renderAuditModule(store) {
  ...
  const deniedPolicies = policies.filter((decision) => decision.result !== "ALLOW");
  ...
  const approvalTrace = approvals.length ? approvals.map((approval) => `<article class="trace-card"><strong>${escapeHtml(approval.decision)}</strong><span>${escapeHtml(approval.subject_type)} ${escapeHtml(shortId(approval.subject_id))}</span><small>Actor ${escapeHtml(shortId(approval.approver_actor_id))} • Authority ${escapeHtml(shortId(approval.authority_id))} • ${escapeHtml(approval.authentication_strength)}</small></article>`).join("") : `<p class="empty">No approvals yet. Approve a proposal to create a governance trace.</p>`;
  const policyTrace = policies.length ? policies.slice(0, 12).map((decision) => `<article class="trace-card ${decision.result === "ALLOW" ? "" : "danger"}"><strong>${escapeHtml(decision.result)}</strong><span>${escapeHtml(decision.action)} on ${escapeHtml(decision.resource_type)}</span><small>${escapeHtml((decision.reasons ?? []).join("; ") || decision.created_at)}</small></article>`).join("") : `<p class="empty">No policy decisions yet.</p>`;
  const membershipTrace = memberships.length ? memberships.slice(0, 12).map((membership) => `<article class="trace-card"><strong>${escapeHtml(membership.role)}</strong><span>${escapeHtml(personName(store, membership.person_id))}</span><small>${escapeHtml(membership.status)} • Actor ${escapeHtml(shortId(membership.actor_id))}</small></article>`).join("") : `<p class="empty">No firm memberships yet.</p>`;
  const authorityTrace = authorities.length ? authorities.slice(0, 12).map((authority) => `<article class="trace-card"><strong>${escapeHtml(authority.authority_type)}</strong><span>${escapeHtml(authority.authority_scope)}</span><small>${escapeHtml(authority.status)} • ${escapeHtml((authority.permitted_actions ?? []).join(", "))}</small></article>`).join("") : `<p class="empty">No professional authorities yet.</p>`;
  document.querySelector("#auditView").innerHTML = `<section class="grid two"><section class="panel"><div class="panel-heading row"><div><h2>Audit Tools</h2><p>Export or reset the local MVP development store.</p></div></div><button id="exportStore" type="button">Export Store JSON</button><button id="resetStore" type="button" class="secondary danger-soft">Reset Local Store</button><p class="form-note">Reset only clears local development JSON data.</p></section><section class="panel"><div class="panel-heading"><h2>Governance Summary</h2><p>Traceability across identity, authority, events, audit, policies, and approvals.</p></div>${renderRelatedContext("Governance counts", [["Events", events.length], ["Audit records", audits.length], ["Policy decisions", policies.length], ["Denied policies", deniedPolicies.length], ["Approvals", approvals.length], ["Memberships", memberships.length], ["Authorities", authorities.filter((authority) => authority.status === "ACTIVE").length]])}${renderRelatedContext("Current actor context", [["Mode", authContext.mode ?? "dev"], ["Actor", shortId(authContext.actor?.actor_id)], ["Authority valid", authContext.authority_valid ? "Yes" : "No"], ["Membership", authContext.membership?.role ?? "Not resolved"]])}</section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Identity & Authority</h2><p>Firm memberships and professional authorities used by Stage 4 trust controls.</p></div><div class="trace-list">${membershipTrace}</div><hr class="soft-divider" /><div class="trace-list">${authorityTrace}</div></section><section class="panel"><div class="panel-heading"><h2>Approval & Policy Trace</h2><p>Explicit approval decisions, allowed policy decisions, and denied attempts.</p></div><div class="trace-list">${approvalTrace}</div><hr class="soft-divider" /><div class="trace-list">${policyTrace}</div></section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Event Timeline</h2><p>Canonical event sequence, newest first.</p></div>${renderTimeline(events.slice(0, 30))}</section><section class="panel"><div class="panel-heading"><h2>Audit Records</h2><p>Material audit records retained for diagnostics.</p></div><details open><summary>Show audit JSON</summary><pre class="output tall">${escapeHtml(JSON.stringify(audits.slice(0,50),null,2))}</pre></details></section></section>`;
  document.querySelector("#exportStore")?.addEventListener("click",()=>downloadJson(`vfirm-store-${new Date().toISOString().slice(0,10)}.json`,store));
  document.querySelector("#resetStore")?.addEventListener("click",async()=>{ if(!confirm("Reset local vFirm store?")) return; await runUiCommand({ label:"Reset local store", button:document.querySelector("#resetStore"), success:"Local store reset", action:async()=>{ await request("/mvp/reset",{method:"POST",body:JSON.stringify({})}); state=defaultState(); await refresh(); switchView("dashboard"); }}); });
}
...
function renderInvoiceModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const invoiceOptions = invoices.map((invoice) => `<option value="${escapeHtml(invoice.id)}">${escapeHtml(invoice.invoice_number)} - ${escapeHtml(invoice.status)} - ${escapeHtml(money((invoice.line_items ?? []).reduce((sum, item) => sum + Number(item.amount ?? 0), 0), invoice.currency))}</option>`).join("");
  const issuable = invoices.filter((invoice) => invoice.status === "DRAFT");
  const payable = invoices.filter((invoice) => invoice.status === "ISSUED");
  const rows = invoices.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Invoice</th><th>Status</th><th>Amount</th><th>Project</th><th>Payments</th><th>Detail</th></tr></thead><tbody>${invoices.map((invoice,index)=>`<tr><td>${escapeHtml(invoice.invoice_number)}</td><td><span class="pill">${escapeHtml(invoice.status)}</span></td><td>${escapeHtml(money((invoice.line_items ?? []).reduce((sum,item)=>sum+Number(item.amount??0),0),invoice.currency))}</td><td>${escapeHtml(shortId(invoice.project_id))}</td><td>${countBy(store.payment_statuses,(payment)=>payment.invoice_id===invoice.id)}</td><td><button class="secondary small" data-invoice-detail="${index}">View</button></td></tr>`).join("")}</tbody></table></div><pre class="output detail-output" id="invoiceDetailBox">Select an invoice.</pre>` : `<p class="empty">No invoices yet. Create one from the Projects tab.</p>`;
  document.querySelector("#invoicesView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Commercial Actions</h2><p>Issue invoices after delivery, then record payment status.</p></div><form id="invoiceIssueForm"><label>Draft invoice<select name="invoice_id" required>${issuable.map((invoice)=>`<option value="${escapeHtml(invoice.id)}">${escapeHtml(invoice.invoice_number)}</option>`).join("")}</select></label><button type="submit" ${issuable.length ? "" : "disabled"}>Issue Invoice</button>${disabledHint(Boolean(issuable.length), "Create a draft invoice first. It can issue only after deliverable issue.")}<p class="form-note">Endpoint: POST /invoices/issue</p></form><hr class="soft-divider" /><form id="paymentRecordForm"><label>Issued invoice<select name="invoice_id" required>${payable.map((invoice)=>`<option value="${escapeHtml(invoice.id)}">${escapeHtml(invoice.invoice_number)}</option>`).join("")}</select></label><label>Amount<input name="amount" type="number" value="2500" min="0" /></label><button type="submit" ${payable.length ? "" : "disabled"}>Record Payment</button>${disabledHint(Boolean(payable.length), "Issue an invoice before recording payment.")}<p class="form-note">Endpoint: POST /payments/record</p></form></section><section class="panel"><div class="panel-heading"><h2>Commercial Summary</h2><p>Invoice and payment status for pilot transaction control.</p></div>${renderRelatedContext("Billing controls", [["Invoices", invoices.length], ["Draft", countBy(invoices,(invoice)=>invoice.status==="DRAFT")], ["Issued", countBy(invoices,(invoice)=>invoice.status==="ISSUED")], ["Paid", countBy(invoices,(invoice)=>invoice.status==="PAID")], ["Payment records", (store.payment_statuses ?? []).length]])}</section></section><section class="panel"><div class="panel-heading"><h2>Invoices</h2><p>Invoice draft, issue, and payment records.</p></div>${rows}</section>`;
  document.querySelector("#invoiceIssueForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const invoice_id=new FormData(event.currentTarget).get("invoice_id"); await runUiCommand({label:"Issue invoice",form:event.currentTarget,success:"Invoice issued",action:async()=>{ const data=await request("/invoices/issue",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,invoice_id,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("invoices"); return data; }}); });
  document.querySelector("#paymentRecordForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record payment",form:event.currentTarget,success:"Payment recorded",action:async()=>{ const data=await request("/payments/record",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,invoice_id:fd.get("invoice_id"),amount:Number(fd.get("amount")||0),currency:"MYR",payment_status:"PAID",actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("invoices"); return data; }}); });
  document.querySelectorAll("button[data-invoice-detail]").forEach((button)=>button.addEventListener("click",()=>{ const invoice=invoices[Number(button.dataset.invoiceDetail)]; const payments=(store.payment_statuses??[]).filter((payment)=>payment.invoice_id===invoice.id); document.querySelector("#invoiceDetailBox").innerHTML=renderHumanDetail(invoice.invoice_number, [["Status",invoice.status],["Amount",money((invoice.line_items??[]).reduce((sum,item)=>sum+Number(item.amount??0),0),invoice.currency)],["Project",shortId(invoice.project_id)],["Due",invoice.due_at],["Payments",payments.length]], {invoice,payments}); }));
}
function renderAiWorkforceModule(store) {
  const contract = activeWorkspaceContract(store);
  const allowedTemplateCodes = workerTemplateCodesForContract(contract);
  const templates = (store.worker_templates ?? []).filter((template) => allowedTemplateCodes.has(template.code));
  const allTemplates = store.worker_templates ?? [];
  const workers = store.worker_instances ?? [];
  const tasks = store.tasks ?? [];
  const activeWorkers = workers.filter((worker) => worker.runtime_status === "ACTIVE");
  const latestFirm = contract.firm;
  const latestTenant = contract.tenant;
  const principalActor = contract.principal;
  const firstTemplate = templates[0];
  const defaultName = defaultWorkerNameForTemplate(firstTemplate?.code, contract);
  const taskOptions = tasks.map((task) => `<option value="${escapeHtml(task.id)}">${escapeHtml(task.task_type)} - ${escapeHtml(task.state)} - ${escapeHtml(shortId(task.id))}</option>`).join("");
  const workerOptions = activeWorkers.map((worker) => `<option value="${escapeHtml(worker.id)}">${escapeHtml(worker.name)} - ${escapeHtml(shortId(worker.id))}</option>`).join("");
  const templateOptions = templates.map((template) => `<option value="${escapeHtml(template.code)}">${escapeHtml(template.name)} ${escapeHtml(template.version)}</option>`).join("");
  const rows = workers.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Worker</th><th>Status</th><th>Template</th><th>Tools</th><th>Budget</th><th>Detail</th></tr></thead><tbody>${workers.map((worker,index)=>{ const template=allTemplates.find((item)=>item.id===worker.worker_template_id); return `<tr><td>${escapeHtml(worker.name)}</td><td><span class="pill">${escapeHtml(worker.runtime_status)}</span></td><td>${escapeHtml(template?.code ?? shortId(worker.worker_template_id))}</td><td>${escapeHtml((worker.tool_allowlist??[]).join(", "))}</td><td>${escapeHtml(worker.budget_envelope?.max_cost ?? "-")} ${escapeHtml(worker.budget_envelope?.currency ?? "")}</td><td><button class="secondary small" data-worker-detail="${index}">View</button></td></tr>`; }).join("")}</tbody></table></div><pre class="output detail-output" id="workerDetailBox">Select a worker.</pre>` : `<p class="empty">No AI workers provisioned yet for ${escapeHtml(latestFirm?.name ?? "the active firm")}.</p>`;
  document.querySelector("#aiWorkforceView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>AI Workforce Actions</h2><p>Provision only workers allowed by ${escapeHtml(contract.subscription?.package_code ?? "the selected workspace profile")}. Workers remain bounded, attributable, and reviewable.</p></div><form id="workerProvisionForm"><label>Template<select name="worker_template_code" required>${templateOptions}</select></label><label>Worker name<input name="name" value="${escapeHtml(defaultName)}" required /></label><button type="submit" ${latestFirm && templates.length ? "" : "disabled"}>Provision Worker</button>${disabledHint(Boolean(latestFirm && templates.length), "No allowed worker templates are bound to this firm workspace.")}<p class="form-note">Endpoint: POST /worker-instances</p></form><form id="workerActivateForm"><label>Worker<select name="worker_instance_id" required>${workers.map((worker)=>`<option value="${escapeHtml(worker.id)}">${escapeHtml(worker.name)} - ${escapeHtml(worker.runtime_status)}</option>`).join("")}</select></label><button type="submit" ${workers.length ? "" : "disabled"}>Activate Worker</button><p class="form-note">Endpoint: POST /worker-instances/activate</p></form><hr class="soft-divider" /><form id="workerAssignForm"><label>Active worker<select name="worker_instance_id" required>${workerOptions}</select></label><label>Task<select name="task_id" required>${taskOptions}</select></label><button type="submit" ${activeWorkers.length && tasks.length ? "" : "disabled"}>Assign Task</button>${disabledHint(Boolean(activeWorkers.length && tasks.length), "Activate a worker and open a project task first.")}<p class="form-note">Endpoint: POST /runtime/tasks/assign-ai</p></form><form id="toolInvokeForm"><label>Allowed tool<input name="tool_name" value="${escapeHtml(contract.profile.firm_type === "ORGANIZATION_SUPPORT" ? "document.register.prepare" : "formwork.input.extract")}" /></label><button type="submit" ${activeWorkers.length && tasks.length ? "" : "disabled"}>Request Tool Invocation</button><p class="form-note">Endpoint: POST /runtime/tool-invocations</p></form><form id="workerOutputForm"><button type="submit" ${activeWorkers.length && tasks.length ? "" : "disabled"}>Produce Reviewable Output</button><p class="form-note">Endpoint: POST /runtime/tasks/output</p></form></section><section class="panel"><div class="panel-heading"><h2>AI Runtime Summary</h2><p>${escapeHtml(latestFirm?.name ?? "Active firm")} workers are filtered by the selected tenant and firm boundary.</p></div>${renderRelatedContext("Runtime controls", [["Allowed templates", templates.length], ["Workers", workers.length], ["Active workers", activeWorkers.length], ["Task outputs", (store.task_outputs ?? []).length], ["Tool invocations", (store.tool_invocations ?? []).length], ["Human review required", countBy(store.task_outputs,(output)=>output.requires_human_review)], ["Authority boundary", "No autonomous regulated approval"]])}</section></section><section class="panel"><div class="panel-heading"><h2>Worker Instances</h2><p>Provisioned AI actors and their envelopes for ${escapeHtml(latestFirm?.name ?? "the selected firm")}.</p></div>${rows}</section>`;
  document.querySelector("#workerProvisionForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Provision AI worker",form:event.currentTarget,success:"AI worker provisioned",action:async()=>{ const data=await request("/worker-instances",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,worker_template_code:fd.get("worker_template_code"),name:fd.get("name"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("ai-workforce"); return data; }}); });
  document.querySelector("#workerActivateForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const worker_instance_id=new FormData(event.currentTarget).get("worker_instance_id"); await runUiCommand({label:"Activate AI worker",form:event.currentTarget,success:"AI worker activated",action:async()=>{ const data=await request("/worker-instances/activate",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,worker_instance_id,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("ai-workforce"); return data; }}); });
  document.querySelector("#workerAssignForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Assign AI task",form:event.currentTarget,success:"Task assigned to AI worker",action:async()=>{ const data=await request("/runtime/tasks/assign-ai",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,worker_instance_id:fd.get("worker_instance_id"),task_id:fd.get("task_id"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("ai-workforce"); return data; }}); });
  document.querySelector("#toolInvokeForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const worker=activeWorkers[0]; const task=tasks.find((item)=>item.assigned_actor_or_worker_ref===worker?.id)??tasks[0]; const tool_name=new FormData(event.currentTarget).get("tool_name"); await runUiCommand({label:"Request AI tool",form:event.currentTarget,success:"Tool invocation requested",action:async()=>{ const data=await request("/runtime/tool-invocations",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,worker_instance_id:worker.id,task_id:task.id,tool_name,input_summary:"Workspace operator requested bounded tool."})}); await refresh(); switchView("ai-workforce"); return data; }}); });
  document.querySelector("#workerOutputForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const worker=activeWorkers[0]; const task=tasks.find((item)=>item.assigned_actor_or_worker_ref===worker?.id)??tasks[0]; await runUiCommand({label:"Produce AI output",form:event.currentTarget,success:"Reviewable AI output produced",action:async()=>{ const data=await request("/runtime/tasks/output",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,worker_instance_id:worker.id,task_id:task.id,output_ref:defaultOutputRef(contract),evidence_refs:[contract.serviceLines[0]?.service_code ?? "workspace_service_evidence"],quality_flags:["requires_human_review"],requires_human_review:true})}); await refresh(); switchView("ai-workforce"); return data; }}); });
  document.querySelectorAll("button[data-worker-detail]").forEach((button)=>button.addEventListener("click",()=>{ const worker=workers[Number(button.dataset.workerDetail)]; const outputs=(store.task_outputs??[]).filter((output)=>output.worker_instance_id===worker.id); const tools=(store.tool_invocations??[]).filter((tool)=>tool.worker_instance_id===worker.id); document.querySelector("#workerDetailBox").innerHTML=renderHumanDetail(worker.name, [["Status",worker.runtime_status],["Tools",(worker.tool_allowlist??[]).join(", ")],["Outputs",outputs.length],["Tool calls",tools.length],["Requires human review","Yes"]], {worker,outputs,tools}); }));
}
function renderOperatorActionCards(actions = []) {
  if (!actions.length) return `<div class="callout success"><strong>No urgent private directory actions.</strong><span>Keep monitoring review board, enquiry, renewal, and audit readiness.</span></div>`;
  const label = (type) => ({ REVIEW_BOARD_DECISION_DUE: "Review Board", PRIVATE_ENQUIRY_FOLLOW_UP: "Private Enquiry", QUALIFICATION_RENEWAL_RISK: "Renewal Risk" }[type] ?? type);
  return `<div class="operator-action-grid">${actions.slice(0, 6).map((action) => `<article class="metric-card warning"><span>${escapeHtml(label(action.type))}</span><strong>${escapeHtml(action.severity ?? "MEDIUM")}</strong><small>${escapeHtml(action.summary ?? "Operator follow-up required.")}</small><code>${escapeHtml(action.listing_id ? `Listing ${shortId(action.listing_id)}` : action.enquiry_id ? `Enquiry ${shortId(action.enquiry_id)}` : action.renewal_review_id ? `Renewal ${shortId(action.renewal_review_id)}` : "Private directory")}</code></article>`).join("")}</div>`;
}

function renderOperatorBoundaryCards() {
  const boundaries = ["No public marketplace", "No live matching", "No ranking", "No capacity allocation", "No VF-24 publication", "No pricing intelligence", "No autonomous award", "No autonomous regulated approval"];
...
function renderNetworkModule(store) {
  ...
  const qualifiedListings = listings.filter((listing) => listing.commercial_model?.directory_type === "CONTROLLED_PRIVATE_QUALIFIED_DIRECTORY");
  const reviewDecisions = store.directory_review_board_decisions ?? [];
  const privateEnquiries = store.directory_private_enquiries ?? [];
  const collaborationRequests = (store.collaboration_requests ?? []).filter((request) => request.metadata?.source_directory_enquiry_id);
  const renewalReviews = store.qualification_renewal_reviews ?? [];
  const qualificationGates = store.network_qualification_gates ?? [];
  const latestFirm = latestRecord(store, "firms");
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const firms = (store.firms ?? []).filter((firm) => !latestTenant || firm.tenant_id === latestTenant.id);
  const passGates = qualificationGates.filter((gate) => (!latestTenant || gate.tenant_id === latestTenant.id) && gate.gate_status === "PASS");
  ...
  const intelligenceSummary = store.private_directory_intelligence_summary ?? {};
  const actorForFirm = (firm) => latestPrincipalActor(store, firm?.id) ?? systemActorForBrowser(firm?.tenant_id ?? latestTenant?.id, firm?.id);
  const authHeadersFor = (actor, firm) => ({
    "x-vfirm-actor-id": actor?.actor_id ?? actor?.id,
    "x-vfirm-tenant-id": actor?.tenant_id ?? firm?.tenant_id ?? latestTenant?.id,
    "x-vfirm-firm-id": actor?.firm_id ?? firm?.id,
    "x-vfirm-role": actor?.role ?? "principal"
  });
  const firmOptions = firms.map((firm) => `<option value="${escapeHtml(firm.id)}">${escapeHtml(firm.name)} - ${escapeHtml(shortId(firm.id))}</option>`).join("");
  const gateOptions = passGates.map((gate) => {
  ...
  const listingOptions = qualifiedListings.map((listing) => `<option value="${escapeHtml(listing.id)}">${escapeHtml(listing.title)} - ${escapeHtml(listing.status)} - ${escapeHtml(shortId(listing.id))}</option>`).join("");
  const enquiryOptions = privateEnquiries.filter((enquiry) => enquiry.status === "ENQUIRY_RECORDED").map((enquiry) => `<option value="${escapeHtml(enquiry.id)}">${escapeHtml(shortId(enquiry.id))} - ${escapeHtml(enquiry.enquiry_summary ?? "Private enquiry")}</option>`).join("");
  const listingRows = qualifiedListings.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Listing</th><th>Provider</th><th>Jurisdiction</th><th>Status</th><th>Detail</th></tr></thead><tbody>${qualifiedListings.map((listing,index)=>{ const firm=firms.find((item)=>item.id===listing.firm_id); const gate=qualificationGates.find((item)=>item.id===listing.commercial_model?.qualification_gate_id); return `<tr><td>${escapeHtml(listing.title)}</td><td>${escapeHtml(firm?.name ?? shortId(listing.firm_id))}</td><td>${escapeHtml(gate?.jurisdiction_ref ?? "-")}</td><td><span class="pill">${escapeHtml(listing.status)}</span></td><td><button class="secondary small" data-directory-listing-detail="${index}">View</button></td></tr>`; }).join("")}</tbody></table></div><pre class="output detail-output" id="directoryListingDetailBox">Select a private directory listing.</pre>` : `<p class="empty">No qualified private directory listings yet. Create a PASS qualification gate before publication.</p>`;
  const reviewRows = reviewDecisions.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Decision</th><th>Listing</th><th>Evidence</th><th>Detail</th></tr></thead><tbody>${reviewDecisions.slice(-10).map((decision,index)=>`<tr><td><span class="pill">${escapeHtml(decision.decision)}</span></td><td>${escapeHtml(shortId(decision.listing_id))}</td><td>${escapeHtml((decision.evidence_refs ?? []).join(", "))}</td><td><button class="secondary small" data-directory-review-detail="${index}">View</button></td></tr>`).join("")}</tbody></table></div><pre class="output detail-output" id="directoryReviewDetailBox">Select a review decision.</pre>` : `<p class="empty">No Directory Review Board decisions recorded yet.</p>`;
  const enquiryRows = privateEnquiries.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Enquiry</th><th>Requester</th><th>Provider</th><th>Status</th><th>Detail</th></tr></thead><tbody>${privateEnquiries.slice(-10).map((enquiry,index)=>{ const requester=firms.find((firm)=>firm.id===enquiry.requesting_firm_id); const provider=firms.find((firm)=>firm.id===enquiry.provider_firm_id); return `<tr><td>${escapeHtml(shortId(enquiry.id))}</td><td>${escapeHtml(requester?.name ?? shortId(enquiry.requesting_firm_id))}</td><td>${escapeHtml(provider?.name ?? shortId(enquiry.provider_firm_id))}</td><td><span class="pill">${escapeHtml(enquiry.status)}</span></td><td><button class="secondary small" data-directory-enquiry-detail="${index}">View</button></td></tr>`; }).join("")}</tbody></table></div><pre class="output detail-output" id="directoryEnquiryDetailBox">Select a private enquiry.</pre>` : `<p class="empty">No private directory enquiries recorded yet.</p>`;
  const renewalRows = renewalReviews.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Review</th><th>Listing</th><th>Expiry</th><th>Evidence</th><th>Detail</th></tr></thead><tbody>${renewalReviews.slice(-10).map((review,index)=>`<tr><td><span class="pill">${escapeHtml(review.review_status)}</span></td><td>${escapeHtml(shortId(review.listing_id))}</td><td>${escapeHtml(review.expires_at ?? "-")}</td><td>${escapeHtml((review.evidence_refs ?? []).join(", "))}</td><td><button class="secondary small" data-directory-renewal-detail="${index}">View</button></td></tr>`).join("")}</tbody></table></div><pre class="output detail-output" id="directoryRenewalDetailBox">Select a renewal review.</pre>` : `<p class="empty">No qualification renewal or expiry reviews recorded yet.</p>`;
  const pendingActionCards = renderOperatorActionCards(intelligenceSummary.pending_actions ?? []);
  const boundaryCards = renderOperatorBoundaryCards();
  document.querySelector("#networkView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Private Directory Operator UI</h2><p>ME-S5 exposes controlled directory publication, review board decisions, manual enquiries, collaboration requests, and renewal monitoring.</p></div><form id="qualifiedDirectoryPublishForm"><label>PASS qualification gate<select name="qualification_gate_id">${gateOptions || '<option value="">No PASS qualification gate available</option>'}</select></label><label>Listing title<input name="title" value="Qualified private Formwork directory service" required /></label><label>Description<input name="description" value="Controlled private directory service publication for trusted network discovery only." required /></label><button type="submit" ${latestTenant && passGates.length ? "" : "disabled"}>Publish Qualified Directory Listing</button>${disabledHint(Boolean(latestTenant && passGates.length), "Create a PASS qualification gate before directory publication.")}<p class="form-note">Endpoint: POST /marketplace/directory-publications</p></form><form id="directoryReviewDecisionForm"><label>Listing<select name="listing_id">${listingOptions || '<option value="">No qualified listing available</option>'}</select></label><label>Decision<select name="decision"><option value="REVIEW_CONTINUE">Review continue</option><option value="SUSPEND">Suspend</option><option value="REVOKE">Revoke</option></select></label><label>Decision summary<input name="decision_summary" value="Listing remains qualified for private directory publication only." required /></label><label>Evidence ref<input name="evidence_ref" value="me-s5-review-board-evidence" required /></label><button type="submit" ${qualifiedListings.length ? "" : "disabled"}>Record Review Board Decision</button><p class="form-note">Endpoint: POST /marketplace/directory-review-board/decisions</p></form></section><section class="panel compact-form"><div class="panel-heading"><h2>Manual Enquiry and Renewal Controls</h2><p>Requests stay private and operator-led. No live matching, ranking, allocation, or autonomous award is available here.</p></div><form id="privateDirectoryEnquiryForm"><label>Listing<select name="listing_id">${listingOptions || '<option value="">No qualified listing available</option>'}</select></label><label>Requesting firm<select name="requesting_firm_id">${firmOptions || '<option value="">No firm available</option>'}</select></label><label>Enquiry summary<input name="enquiry_summary" value="Manual enquiry about qualified Formwork support." required /></label><button type="submit" ${qualifiedListings.length && firms.length > 1 ? "" : "disabled"}>Record Private Enquiry</button>${disabledHint(Boolean(qualifiedListings.length && firms.length > 1), "A private enquiry needs a qualified listing and a separate requesting firm.")}<p class="form-note">Endpoint: POST /marketplace/private-directory/enquiries</p></form><form id="directoryEnquiryCollaborationForm"><label>Recorded enquiry<select name="enquiry_id">${enquiryOptions || '<option value="">No open enquiry available</option>'}</select></label><label>Request summary<input name="request_summary" value="Manual collaboration request after private directory enquiry." required /></label><button type="submit" ${enquiryOptions ? "" : "disabled"}>Request Collaboration</button><p class="form-note">Endpoint: POST /marketplace/private-directory/enquiries/request-collaboration</p></form><form id="qualificationRenewalReviewForm"><label>Listing<select name="listing_id">${listingOptions || '<option value="">No qualified listing available</option>'}</select></label><label>Review status<select name="review_status"><option value="VALID">Valid</option><option value="EXPIRING">Expiring</option><option value="RENEWAL_REQUIRED">Renewal required</option><option value="SUSPEND_PUBLICATION">Suspend publication</option></select></label><label>Expires at<input name="expires_at" type="date" value="2026-12-31" /></label><label>Next review due<input name="next_review_due_at" type="date" value="2026-11-30" /></label><label>Evidence ref<input name="evidence_ref" value="me-s5-renewal-evidence" required /></label><button type="submit" ${qualifiedListings.length ? "" : "disabled"}>Record Renewal Review</button><p class="form-note">Endpoint: POST /marketplace/qualification-renewal-reviews</p></form></section></section><section class="panel"><div class="panel-heading"><h2>Controlled Directory Summary</h2><p>Operator view for private qualified directory governance. Public marketplace behaviors remain out of scope.</p></div>${renderRelatedContext("ME-S5 operator evidence", [["Qualified listings", qualifiedListings.length], ["PASS qualification gates", passGates.length], ["Review board decisions", reviewDecisions.length], ["Private enquiries", privateEnquiries.length], ["Manual collaboration requests", collaborationRequests.length], ["Renewal reviews", renewalReviews.length], ["ME-S2 status", qualifiedSummary.status ?? "Not yet ready"], ["ME-S3 status", governanceSummary.status ?? "Not yet ready"], ["ME-S6 status", intelligenceSummary.status ?? "Not yet ready"], ["Pending actions", intelligenceSummary.counts?.pending_actions ?? 0], ["Renewal risks", intelligenceSummary.counts?.renewal_risks ?? 0], ["Audit readiness", intelligenceSummary.readiness?.audit_events ?? 0]])}${renderRelatedContext("Boundary lock", [["Directory type", "Controlled private qualified directory"], ["Listing approval", "Human governance required"], ["Discovery", "Manual enquiry only"], ["Forbidden", "public marketplace / live matching / ranking / allocation / VF-24 publication / pricing intelligence / autonomous award"]])}<div class="operator-walkthrough-panel"><h3>Operator next actions</h3><p>Use these cards for rehearsal: review pending board items, follow up enquiries manually, and resolve renewal risk before private directory operation.</p>${pendingActionCards}</div><div class="operator-walkthrough-panel"><h3>Forbidden boundary reminders</h3>${boundaryCards}</div><details open><summary>ME-S6 private readiness summary</summary><pre class="output">${escapeHtml(JSON.stringify(intelligenceSummary.pending_actions ? { status: intelligenceSummary.status, readiness: intelligenceSummary.readiness, pending_actions: intelligenceSummary.pending_actions, boundaries: intelligenceSummary.boundaries } : { message: "No ME-S6 readiness summary yet." }, null, 2))}</pre></details></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Qualified Directory Listings</h2><p>Human-approved, tenant-confidential service publications only.</p></div>${listingRows}</section><section class="panel"><div class="panel-heading"><h2>Directory Review Board Decisions</h2><p>Review, suspension, and revocation decisions are auditable and evidence-backed.</p></div>${reviewRows}</section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Private Directory Enquiries</h2><p>Manual enquiry records; no automated matching or price-first ranking.</p></div>${enquiryRows}</section><section class="panel"><div class="panel-heading"><h2>Qualification Renewal Monitoring</h2><p>Expiry and renewal reviews keep listings under controlled governance.</p></div>${renewalRows}</section></section>`;
  document.querySelector("#qualifiedDirectoryPublishForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Publish qualified directory listing",form:event.currentTarget,success:"Qualified directory listing published",action:async()=>{ const gate=passGates.find((item)=>item.id===fd.get("qualification_gate_id")); const firm=firms.find((item)=>item.id===gate?.provider_firm_id); const actor=actorForFirm(firm); const data=await request("/marketplace/directory-publications",{method:"POST",headers:authHeadersFor(actor,firm),body:JSON.stringify({tenant_id:latestTenant.id,firm_id:firm.id,qualification_gate_id:gate.id,title:fd.get("title"),description:fd.get("description"),actor})}); await refresh(); switchView("network"); return data; }}); });
  document.querySelector("#directoryReviewDecisionForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record directory review board decision",form:event.currentTarget,success:"Review board decision recorded",action:async()=>{ const listing=qualifiedListings.find((item)=>item.id===fd.get("listing_id")); const firm=firms.find((item)=>item.id===listing?.firm_id); const actor=actorForFirm(firm); const data=await request("/marketplace/directory-review-board/decisions",{method:"POST",headers:authHeadersFor(actor,firm),body:JSON.stringify({tenant_id:latestTenant.id,provider_firm_id:firm.id,listing_id:listing.id,decision:fd.get("decision"),decision_summary:fd.get("decision_summary"),evidence_refs:[fd.get("evidence_ref")],actor})}); await refresh(); switchView("network"); return data; }}); });
  document.querySelector("#privateDirectoryEnquiryForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record private directory enquiry",form:event.currentTarget,success:"Private directory enquiry recorded",action:async()=>{ const listing=qualifiedListings.find((item)=>item.id===fd.get("listing_id")); const requester=firms.find((item)=>item.id===fd.get("requesting_firm_id")); if(requester?.id===listing?.firm_id) throw new Error("Choose a requesting firm separate from the provider firm."); const actor=actorForFirm(requester); const data=await request("/marketplace/private-directory/enquiries",{method:"POST",headers:authHeadersFor(actor,requester),body:JSON.stringify({tenant_id:latestTenant.id,requesting_firm_id:requester.id,provider_firm_id:listing.firm_id,listing_id:listing.id,enquiry_summary:fd.get("enquiry_summary"),actor})}); await refresh(); switchView("network"); return data; }}); });
  document.querySelector("#directoryEnquiryCollaborationForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Request collaboration from private enquiry",form:event.currentTarget,success:"Manual collaboration request recorded",action:async()=>{ const enquiry=privateEnquiries.find((item)=>item.id===fd.get("enquiry_id")); const firm=firms.find((item)=>item.id===enquiry?.requesting_firm_id); const actor=actorForFirm(firm); const data=await request("/marketplace/private-directory/enquiries/request-collaboration",{method:"POST",headers:authHeadersFor(actor,firm),body:JSON.stringify({tenant_id:latestTenant.id,requesting_firm_id:firm.id,enquiry_id:enquiry.id,request_summary:fd.get("request_summary"),actor})}); await refresh(); switchView("network"); return data; }}); });
  document.querySelector("#qualificationRenewalReviewForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record qualification renewal review",form:event.currentTarget,success:"Qualification renewal review recorded",action:async()=>{ const listing=qualifiedListings.find((item)=>item.id===fd.get("listing_id")); const gate=qualificationGates.find((item)=>item.id===listing?.commercial_model?.qualification_gate_id); const firm=firms.find((item)=>item.id===listing?.firm_id); const actor=actorForFirm(firm); const data=await request("/marketplace/qualification-renewal-reviews",{method:"POST",headers:authHeadersFor(actor,firm),body:JSON.stringify({tenant_id:latestTenant.id,provider_firm_id:firm.id,qualification_gate_id:gate.id,listing_id:listing.id,review_status:fd.get("review_status"),expires_at:fd.get("expires_at") || null,next_review_due_at:fd.get("next_review_due_at") || null,evidence_refs:[fd.get("evidence_ref")],actor})}); await refresh(); switchView("network"); return data; }}); });
  document.querySelectorAll("button[data-directory-listing-detail]").forEach((button)=>button.addEventListener("click",()=>{ const listing=qualifiedListings[Number(button.dataset.directoryListingDetail)]; document.querySelector("#directoryListingDetailBox").innerHTML=renderHumanDetail(listing.title, [["Status",listing.status],["Scope",listing.listing_scope],["Visibility",listing.visibility],["Gate",shortId(listing.commercial_model?.qualification_gate_id)],["Provider",shortId(listing.firm_id)]], {listing}); }));
  document.querySelectorAll("button[data-directory-review-detail]").forEach((button)=>button.addEventListener("click",()=>{ const decision=reviewDecisions.slice(-10)[Number(button.dataset.directoryReviewDetail)]; document.querySelector("#directoryReviewDetailBox").innerHTML=renderHumanDetail("Review board decision", [["Decision",decision.decision],["Listing",shortId(decision.listing_id)],["Provider",shortId(decision.provider_firm_id)],["Evidence",(decision.evidence_refs ?? []).join(", ")]], {decision}); }));
  document.querySelectorAll("button[data-directory-enquiry-detail]").forEach((button)=>button.addEventListener("click",()=>{ const enquiry=privateEnquiries.slice(-10)[Number(button.dataset.directoryEnquiryDetail)]; document.querySelector("#directoryEnquiryDetailBox").innerHTML=renderHumanDetail("Private directory enquiry", [["Status",enquiry.status],["Requester",shortId(enquiry.requesting_firm_id)],["Provider",shortId(enquiry.provider_firm_id)],["Listing",shortId(enquiry.listing_id)]], {enquiry}); }));
  document.querySelectorAll("button[data-directory-renewal-detail]").forEach((button)=>button.addEventListener("click",()=>{ const review=renewalReviews.slice(-10)[Number(button.dataset.directoryRenewalDetail)]; document.querySelector("#directoryRenewalDetailBox").innerHTML=renderHumanDetail("Qualification renewal review", [["Status",review.review_status],["Listing",shortId(review.listing_id)],["Gate",shortId(review.qualification_gate_id)],["Next review",review.next_review_due_at ?? "-"]], {review}); }));
}
function renderOpsModule(store) {
  ...
  const statusClass = readiness.status === "PRODUCTION_READY_CANDIDATE" ? "ok" : readiness.status === "NOT_READY" ? "danger" : "warn";
  const checklist = checks.length ? checks.map((check) => `<article class="trace-card ${check.status === "PASS" ? "" : "danger"}"><strong>${escapeHtml(check.key)}: ${escapeHtml(check.status)}</strong><span>${escapeHtml(check.detail)}</span></article>`).join("") : `<p class="empty">No readiness checks returned yet.</p>`;
  const incidentOptions = incidents.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.title)} - ${escapeHtml(item.status)}</option>`).join("");
  const incidentRows = incidents.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Incident</th><th>Severity</th><th>Status</th><th>Source</th><th>Updated</th></tr></thead><tbody>${incidents.map((item)=>`<tr><td>${escapeHtml(item.title)}<br/><small>${escapeHtml(item.impact_summary ?? item.description ?? "")}</small></td><td>${escapeHtml(item.severity)}</td><td><span class="pill">${escapeHtml(item.status)}</span></td><td>${escapeHtml(item.detection_source)}</td><td>${escapeHtml(item.updated_at ?? item.created_at)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No pilot incidents recorded yet.</p>`;
  document.querySelector("#opsView").innerHTML = `<section class="grid two"><section class="panel"><div class="panel-heading"><h2>Pilot Control Tower</h2><p>Stage 15 pilot observability, incident response, support health, and operator metrics.</p></div>${renderRelatedContext("Operator metrics", [["Status", metrics.status ?? "PILOT_STABLE"], ["Active incidents", metrics.counts?.active_incidents ?? countBy(incidents,(item)=>!["RESOLVED","CLOSED"].includes(item.status))], ["Critical incidents", metrics.counts?.critical_incidents ?? countBy(incidents,(item)=>["SEV1","SEV2"].includes(item.severity) && !["RESOLVED","CLOSED"].includes(item.status))], ["Open support cases", metrics.counts?.open_support_cases ?? countBy(store.support_cases,(item)=>item.status!=="CLOSED")], ["Events", metrics.counts?.events ?? (store.event_log ?? []).length], ["Open tasks", metrics.counts?.open_tasks ?? countBy(store.tasks,(item)=>!["COMPLETED","DONE"].includes(item.state))]])}<div class="checklist warning">${(metrics.warnings ?? []).map((item)=>`<span>${escapeHtml(item)}</span>`).join("") || "<span>No active operator warnings.</span>"}</div></section><section class="panel compact-form"><div class="panel-heading"><h2>Incident Response</h2><p>Open and resolve tenant-scoped pilot incidents.</p></div><form id="incidentOpenForm"><label>Title<input name="title" value="Pilot workflow observation" required /></label><label>Severity<select name="severity"><option value="SEV3">SEV3</option><option value="SEV2">SEV2</option><option value="SEV1">SEV1</option></select></label><label>Impact<input name="impact_summary" value="Operator noticed a pilot workflow issue requiring review." /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Open Incident</button>${disabledHint(Boolean(latestTenant), "Create a tenant before opening incidents.")}<p class="form-note">Endpoint: POST /ops/incidents</p></form><form id="incidentResolveForm"><label>Incident<select name="incident_id">${incidentOptions}</select></label><label>Mitigation<input name="mitigation_summary" value="Mitigation recorded by pilot operator." /></label><label>Root cause<input name="root_cause_summary" value="Root cause reviewed during pilot operations." /></label><button type="submit" ${incidents.length ? "" : "disabled"}>Resolve Incident</button><p class="form-note">Endpoint: POST /ops/incidents/update</p></form></section></section><section class="panel"><div class="panel-heading"><h2>Incident Queue</h2><p>Operational incidents for the controlled pilot.</p></div>${incidentRows}</section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Production Readiness</h2><p>Stage 9 release, environment, persistence, and operations guardrails.</p></div>${renderRelatedContext("Readiness status", [["Status", readiness.status ?? "Unknown"], ["Stage", readiness.stage ?? "Stage 9"], ["API port", readiness.environment?.api_port ?? "-"], ["Port family", readiness.environment?.port_family ?? "309#"], ["Store backend", readiness.persistence?.store_backend ?? "unknown"], ["Release command", readiness.release_gate?.command ?? "npm run check:production-readiness"]])}<p class="form-note ${statusClass}">Local MVP may show warnings until external auth, production database URL, explicit allowed origins, backup policy, and release channel are configured.</p></section><section class="panel"><div class="panel-heading"><h2>Release Gate</h2><p>What must be true before a real production launch.</p></div><div class="checklist evidence">${(readiness.release_gate?.required_before_real_production ?? []).map((item)=>`<span>${escapeHtml(item)}</span>`).join("")}</div><hr class="soft-divider" /><pre class="output">${escapeHtml(JSON.stringify(readiness.environment ?? {}, null, 2))}</pre></section></section><section class="panel"><div class="panel-heading"><h2>Readiness Checks</h2><p>PASS/WARN/FAIL signals from the running API.</p></div><div class="trace-list">${checklist}</div></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Staging Deployment Package</h2><p>Provider-neutral package for staging release preparation.</p></div>${renderRelatedContext("Staging package", [["Code", stagingPackage.code ?? "-"], ["Release channel", stagingPackage.release_channel ?? "-"], ["Services", (stagingPackage.recommended_services ?? []).join(", ")], ["Web port", stagingPackage.local_ports?.web ?? 3090], ["API port", stagingPackage.local_ports?.api ?? "-"]])}<div class="checklist evidence">${(stagingPackage.preflight_commands ?? []).map((item)=>`<span>${escapeHtml(item)}</span>`).join("")}</div></section><section class="panel"><div class="panel-heading"><h2>Data Protection</h2><p>Staging/pilot data classification, export, backup, and retention posture.</p></div>${renderRelatedContext("Data policy", [["Status", dataPolicy.status ?? "-"], ["Default", dataPolicy.default_classification ?? "CONFIDENTIAL"], ["Tenant isolation", dataPolicy.tenant_isolation ?? "required"], ["Backup", dataPolicy.backup_policy ?? "-"], ["Export format", dataPolicy.export_policy?.format ?? "json"], ["Secrets excluded", exportManifest.integrity?.secrets_excluded ? "Yes" : "Required"]])}<details><summary>Export manifest</summary><pre class="output">${escapeHtml(JSON.stringify(exportManifest, null, 2))}</pre></details></section></section>`;
  const opsHost = document.querySelector("#opsView");
  const exceptionRows = (daily.exceptions ?? []).length ? `<div class="trace-list">${daily.exceptions.map((item)=>`<article class="trace-card danger"><strong>${escapeHtml(item.key)}: ${escapeHtml(item.severity)}</strong><span>${escapeHtml(item.detail)}</span></article>`).join("")}</div>` : `<p class="empty">No high-priority daily exceptions.</p>`;
  const handoffRows = handoffs.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Handoff</th><th>Status</th><th>Accepted</th></tr></thead><tbody>${handoffs.map((item)=>`<tr><td>${escapeHtml(item.rehearsal_ref)}</td><td><span class="pill">${escapeHtml(item.handoff_status)}</span></td><td>${escapeHtml(item.accepted_at)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No pilot handoff acceptance recorded yet.</p>`;
  opsHost.innerHTML = `<section class="grid two"><section class="panel"><div class="panel-heading"><h2>Today</h2><p>SF-S6 daily priorities across front desk, admin, sales, technical delivery, projects, and cash.</p></div>${renderRelatedContext("Daily operations", [["Status", daily.status ?? "Waiting for firm data"], ["Open enquiries", daily.counts?.open_enquiries ?? 0], ["Open deadlines", daily.counts?.open_deadlines ?? 0], ["Pending approvals", daily.counts?.pending_approvals ?? 0], ["Open projects", daily.counts?.open_projects ?? 0], ["Blocked packages", daily.counts?.blocked_delivery_packages ?? 0], ["Outstanding", money(daily.cash?.outstanding ?? 0, daily.cash?.currency ?? "MYR")]])}</section><section class="panel"><div class="panel-heading"><h2>Exceptions</h2><p>Items that need principal attention before or during the pilot week.</p></div>${exceptionRows}</section></section>` + opsHost.innerHTML + `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Pilot Handoff</h2><p>Record human acceptance after representative working-week rehearsal.</p></div><form id="pilotHandoffForm"><label>Rehearsal ref<input name="rehearsal_ref" value="SF-S6-WORKING-WEEK-LOCAL" /></label><label>Decision summary<input name="decision_summary" value="SF-S6 daily operations and pilot handoff accepted for controlled local pilot use." /></label><button type="submit" ${latestTenant&&latestFirm?"":"disabled"}>Accept Handoff</button>${disabledHint(Boolean(latestTenant&&latestFirm), "Create a firm before accepting pilot handoff.")}<p class="form-note">Endpoint: POST /pilot/handoff</p></form></section><section class="panel"><div class="panel-heading"><h2>Handoff Register</h2><p>Attributable pilot acceptance records.</p></div>${handoffRows}</section></section>`;
  document.querySelector("#incidentOpenForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Open pilot incident",form:event.currentTarget,success:"Pilot incident opened",action:async()=>{ const data=await request("/ops/incidents",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,title:fd.get("title"),severity:fd.get("severity"),impact_summary:fd.get("impact_summary"),description:fd.get("impact_summary"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("ops"); return data; }}); });
  document.querySelector("#incidentResolveForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Resolve pilot incident",form:event.currentTarget,success:"Pilot incident resolved",action:async()=>{ const data=await request("/ops/incidents/update",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,incident_id:fd.get("incident_id"),status:"RESOLVED",mitigation_summary:fd.get("mitigation_summary"),root_cause_summary:fd.get("root_cause_summary"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("ops"); return data; }}); });
  document.querySelector("#pilotHandoffForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Accept pilot handoff",form:event.currentTarget,success:"Pilot handoff accepted",action:async()=>{ const data=await request("/pilot/handoff",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm.id,rehearsal_ref:fd.get("rehearsal_ref"),decision_summary:fd.get("decision_summary"),evidence_refs:["docs/10_post_freeze_technical_design/SF_S6_DAILY_OPERATIONS_AND_PILOT_HANDOFF_TECHNICAL_DESIGN_v1.0.md"],actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm.id)})}); await refresh(); switchView("ops"); return data; }}); });
}
...
function renderUsersModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const inviteOptions = users.map((user)=>`<option value="${escapeHtml(user.id)}">${escapeHtml(user.email)} - ${escapeHtml(user.invite_status)}</option>`).join("");
  const rows = users.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>User</th><th>Role</th><th>Status</th><th>Provider</th><th>Subject</th></tr></thead><tbody>${users.map((user)=>`<tr><td>${escapeHtml(user.display_name)}<br/><small>${escapeHtml(user.email)}</small></td><td>${escapeHtml(user.pilot_role)}</td><td><span class="pill">${escapeHtml(user.invite_status)}</span></td><td>${escapeHtml(user.auth_provider ?? "-")}</td><td>${escapeHtml(user.external_subject ?? "-")}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No pilot users invited yet.</p>`;
  document.querySelector("#usersView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Pilot User Management</h2><p>Invite and activate users for the controlled staging pilot.</p></div><form id="pilotInviteForm"><label>Email<input name="email" type="email" value="pilot.operator@example.com" required /></label><label>Display name<input name="display_name" value="Pilot Operator" required /></label><label>Role<select name="pilot_role"><option value="PILOT_OPERATOR">Pilot Operator</option><option value="PILOT_PRINCIPAL">Pilot Principal</option><option value="PILOT_OBSERVER">Pilot Observer</option></select></label><button type="submit" ${latestTenant ? "" : "disabled"}>Invite Pilot User</button>${disabledHint(Boolean(latestTenant), "Create a tenant before inviting pilot users.")}<p class="form-note">Endpoint: POST /pilot/users/invite</p></form><form id="pilotActivateForm"><label>Pilot user<select name="pilot_user_id" required>${inviteOptions}</select></label><label>External subject<input name="external_subject" value="staging-user-001" /></label><button type="submit" ${users.length ? "" : "disabled"}>Activate Pilot User</button><p class="form-note">Endpoint: POST /pilot/users/activate</p></form></section><section class="panel"><div class="panel-heading"><h2>Staging Auth Bridge</h2><p>Stage 11 resolves external identity shape without binding to a final provider yet.</p></div>${renderRelatedContext("Pilot access", [["Invited users", users.length], ["Active users", countBy(users,(user)=>user.invite_status==="ACTIVE")], ["Provider mode", "staging-header"], ["Tenant", shortId(latestTenant?.id)], ["Firm", shortId(latestFirm?.id)]])}<p class="form-note">Use headers: x-vfirm-user-email, x-vfirm-user-subject, x-vfirm-auth-provider.</p></section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Auth Provider Adapter</h2><p>Provider-neutral seam for Clerk/Auth0/Supabase/Entra integration.</p></div>${renderRelatedContext("Provider config", [["Provider", authProvider.provider ?? "dev-header"], ["Mode", authProvider.mode ?? "development"], ["Adapter", authProvider.adapter_status ?? "DEV_HEADER_ONLY"], ["Issuer", authProvider.issuer_configured ? "Configured" : "Missing"], ["JWKS", authProvider.jwks_configured ? "Configured" : "Missing"], ["Audience", authProvider.audience_configured ? "Configured" : "Missing"]])}</section><section class="panel"><div class="panel-heading"><h2>Tenant Admin Policy</h2><p>Role/action surface for pilot user administration.</p></div><pre class="output">${escapeHtml(JSON.stringify(adminPolicy.roles ?? {}, null, 2))}</pre></section></section><section class="panel"><div class="panel-heading"><h2>Pilot Users</h2><p>Controlled users registered for staging/pilot access.</p></div>${rows}</section>`;
  document.querySelector("#pilotInviteForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Invite pilot user",form:event.currentTarget,success:"Pilot user invited",action:async()=>{ const data=await request("/pilot/users/invite",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,email:fd.get("email"),display_name:fd.get("display_name"),pilot_role:fd.get("pilot_role"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("users"); return data; }}); });
  document.querySelector("#pilotActivateForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Activate pilot user",form:event.currentTarget,success:"Pilot user activated",action:async()=>{ const data=await request("/pilot/users/activate",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,pilot_user_id:fd.get("pilot_user_id"),external_subject:fd.get("external_subject"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("users"); return data; }}); });
}
function renderSupportModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const userOptions = users.map((user)=>`<option value="${escapeHtml(user.id)}">${escapeHtml(user.email)} - ${escapeHtml(user.invite_status)}</option>`).join("");
  const caseOptions = cases.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.subject)} - ${escapeHtml(item.status)}</option>`).join("");
  const rows = cases.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Case</th><th>Type</th><th>Severity</th><th>Status</th><th>User</th></tr></thead><tbody>${cases.map((item)=>`<tr><td>${escapeHtml(item.subject)}<br/><small>${escapeHtml(item.description ?? "")}</small></td><td>${escapeHtml(item.case_type)}</td><td>${escapeHtml(item.severity)}</td><td><span class="pill">${escapeHtml(item.status)}</span></td><td>${escapeHtml(shortId(item.related_pilot_user_id))}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No support cases yet.</p>`;
  document.querySelector("#supportView").innerHTML = `<section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Support Desk Controls</h2><p>Open/close pilot support cases and revoke pilot access when needed.</p></div><form id="supportCaseForm"><label>Related pilot user<select name="related_pilot_user_id"><option value="">None</option>${userOptions}</select></label><label>Subject<input name="subject" value="Pilot access support request" required /></label><label>Severity<select name="severity"><option value="NORMAL">Normal</option><option value="HIGH">High</option><option value="CRITICAL">Critical</option></select></label><label>Description<input name="description" value="Operator needs help during pilot workflow." /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Open Support Case</button><p class="form-note">Endpoint: POST /support/cases</p></form><form id="supportCloseForm"><label>Open case<select name="support_case_id">${caseOptions}</select></label><label>Resolution<input name="resolution_summary" value="Resolved during pilot support review." /></label><button type="submit" ${cases.length ? "" : "disabled"}>Close Support Case</button><p class="form-note">Endpoint: POST /support/cases/update</p></form><form id="pilotRevokeForm"><label>Pilot user<select name="pilot_user_id">${userOptions}</select></label><label>Reason<input name="revocation_reason" value="pilot_access_revoked_by_support" /></label><button type="submit" ${users.length ? "" : "disabled"}>Revoke Pilot User</button><p class="form-note">Endpoint: POST /pilot/users/revoke</p></form></section><section class="panel"><div class="panel-heading"><h2>Support Queue Summary</h2><p>Tenant-scoped support and access control status.</p></div>${renderRelatedContext("Support controls", [["Pilot users", summary.counts?.pilot_users ?? users.length], ["Active users", summary.counts?.active_pilot_users ?? countBy(users,(user)=>user.invite_status==="ACTIVE")], ["Revoked users", summary.counts?.revoked_pilot_users ?? countBy(users,(user)=>user.invite_status==="REVOKED")], ["Support cases", summary.counts?.support_cases ?? cases.length], ["Open cases", summary.counts?.open_cases ?? countBy(cases,(item)=>item.status!=="CLOSED")], ["Critical cases", summary.counts?.critical_cases ?? countBy(cases,(item)=>item.severity==="CRITICAL")]])}</section></section><section class="panel"><div class="panel-heading"><h2>Support Cases</h2><p>Support desk queue for pilot operations.</p></div>${rows}</section>`;
  document.querySelector("#supportCaseForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Open support case",form:event.currentTarget,success:"Support case opened",action:async()=>{ const data=await request("/support/cases",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,related_pilot_user_id:fd.get("related_pilot_user_id")||null,subject:fd.get("subject"),severity:fd.get("severity"),description:fd.get("description"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("support"); return data; }}); });
  document.querySelector("#supportCloseForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Close support case",form:event.currentTarget,success:"Support case closed",action:async()=>{ const data=await request("/support/cases/update",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,support_case_id:fd.get("support_case_id"),status:"CLOSED",resolution_summary:fd.get("resolution_summary"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("support"); return data; }}); });
  document.querySelector("#pilotRevokeForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Revoke pilot user",form:event.currentTarget,success:"Pilot user revoked",action:async()=>{ const data=await request("/pilot/users/revoke",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,pilot_user_id:fd.get("pilot_user_id"),revocation_reason:fd.get("revocation_reason"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("support"); return data; }}); });
}
function renderReviewBoardModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const reportOptions = reports.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.report_scope)} - ${escapeHtml(item.report_status)}</option>`).join("");
  const boardOptions = boards.filter((item)=>item.review_status==="OPEN").map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.board_name)} - ${escapeHtml(item.review_status)}</option>`).join("");
  const reportRows = reports.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Report</th><th>Status</th><th>Readiness</th><th>Created</th></tr></thead><tbody>${reports.map((item)=>`<tr><td>${escapeHtml(item.report_scope)}<br/><small>${escapeHtml(shortId(item.id))}</small></td><td><span class="pill">${escapeHtml(item.report_status)}</span></td><td>${escapeHtml(item.summary?.readiness ?? "-")}</td><td>${escapeHtml(item.created_at)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No pilot report packs generated yet.</p>`;
  const decisionRows = decisions.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Decision</th><th>Summary</th><th>Next stage</th><th>Date</th></tr></thead><tbody>${decisions.map((item)=>`<tr><td><span class="pill">${escapeHtml(item.decision)}</span></td><td>${escapeHtml(item.decision_summary ?? "-")}</td><td>${escapeHtml(item.next_stage ?? "-")}</td><td>${escapeHtml(item.decided_at)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No stakeholder decisions recorded yet.</p>`;
  document.querySelector("#reviewBoardView").innerHTML = `<section class="grid two"><section class="panel"><div class="panel-heading"><h2>Stakeholder Review Board</h2><p>Stage 17 pilot reporting, export pack, and board decision controls.</p></div>${renderRelatedContext("Board summary", [["Status", summary.status ?? "REPORT_REQUIRED"], ["Report packs", summary.counts?.report_packs ?? reports.length], ["Open boards", summary.counts?.open_boards ?? countBy(boards,(item)=>item.review_status==="OPEN")], ["Decisions", summary.counts?.decisions ?? decisions.length], ["Open improvements", summary.counts?.open_improvements ?? countBy(store.pilot_improvement_items,(item)=>!["DONE","CLOSED"].includes(item.status))], ["Active incidents", summary.counts?.active_incidents ?? countBy(store.pilot_incidents,(item)=>!["RESOLVED","CLOSED"].includes(item.status))]])}</section><section class="panel compact-form"><div class="panel-heading"><h2>Report Pack</h2><p>Generate tenant-scoped pilot report/export manifest.</p></div><form id="reportPackForm"><label>Scope<input name="report_scope" value="FORMWORK_PILOT" /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Generate Report Pack</button>${disabledHint(Boolean(latestTenant), "Create a tenant before generating a pilot report pack.")}<p class="form-note">Endpoint: POST /pilot/report-packs</p></form><form id="reviewBoardForm"><label>Report pack<select name="report_pack_id"><option value="">Latest / none</option>${reportOptions}</select></label><label>Board name<input name="board_name" value="Pilot Stakeholder Review Board" /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Open Review Board</button><p class="form-note">Endpoint: POST /stakeholder-review/boards</p></form></section></section><section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Board Decision</h2><p>Record explicit pilot continuation or expansion decision.</p></div><form id="reviewDecisionForm"><label>Open board<select name="board_id">${boardOptions}</select></label><label>Decision<select name="decision"><option value="APPROVE_EXPANSION">Approve Expansion</option><option value="CONDITIONAL_CONTINUE">Conditional Continue</option><option value="HOLD">Hold</option><option value="STOP">Stop</option></select></label><label>Summary<input name="decision_summary" value="Pilot reviewed and approved for next controlled stage." /></label><button type="submit" ${boards.some((item)=>item.review_status==="OPEN") ? "" : "disabled"}>Record Decision</button><p class="form-note">Endpoint: POST /stakeholder-review/decisions</p></form></section><section class="panel"><div class="panel-heading"><h2>Latest Export Manifest</h2><p>What the report pack includes for stakeholder review.</p></div><pre class="output">${escapeHtml(JSON.stringify(summary.latest_report?.export_manifest ?? reports.at(-1)?.export_manifest ?? {}, null, 2))}</pre></section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Report Packs</h2><p>Generated pilot reporting packs.</p></div>${reportRows}</section><section class="panel"><div class="panel-heading"><h2>Board Decisions</h2><p>Explicit stakeholder review outcomes.</p></div>${decisionRows}</section></section>`;
  document.querySelector("#reportPackForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Generate report pack",form:event.currentTarget,success:"Pilot report pack generated",action:async()=>{ const data=await request("/pilot/report-packs",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,report_scope:fd.get("report_scope"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("review-board"); return data; }}); });
  document.querySelector("#reviewBoardForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Open review board",form:event.currentTarget,success:"Stakeholder review board opened",action:async()=>{ const data=await request("/stakeholder-review/boards",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,report_pack_id:fd.get("report_pack_id")||reports.at(-1)?.id||null,board_name:fd.get("board_name"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("review-board"); return data; }}); });
  document.querySelector("#reviewDecisionForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record board decision",form:event.currentTarget,success:"Board decision recorded",action:async()=>{ const data=await request("/stakeholder-review/decisions",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,board_id:fd.get("board_id"),decision:fd.get("decision"),decision_summary:fd.get("decision_summary"),next_stage:"Stage 18",actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("review-board"); return data; }}); });
}
function renderExpansionModule(store) {
  ...
  const latestTenant = latestFirm ? store.tenants?.find((tenant) => tenant.id === latestFirm.tenant_id) : latestRecord(store, "tenants");
  const principalActor = latestFirm ? latestPrincipalActor(store, latestFirm.id) : null;
  const expansionDecisionOptions = decisions.filter((item)=>item.decision==="APPROVE_EXPANSION").map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.decision)} - ${escapeHtml(shortId(item.id))}</option>`).join("");
  const cohortOptions = cohorts.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.cohort_name)} - ${escapeHtml(item.expansion_status)}</option>`).join("");
  const planOptions = plans.map((item)=>`<option value="${escapeHtml(item.id)}">${escapeHtml(shortId(item.id))} - ${escapeHtml(item.onboarding_status)}</option>`).join("");
  const cohortRows = cohorts.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>Cohort</th><th>Status</th><th>Tenants</th><th>Users</th></tr></thead><tbody>${cohorts.map((item)=>`<tr><td>${escapeHtml(item.cohort_name)}<br/><small>${escapeHtml(shortId(item.id))}</small></td><td><span class="pill">${escapeHtml(item.expansion_status)}</span></td><td>${escapeHtml(item.max_tenants)}</td><td>${escapeHtml(item.max_pilot_users)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No expansion cohorts yet.</p>`;
  const gateRows = gates.length ? `<div class="record-table-wrap"><table class="record-table"><thead><tr><th>RC</th><th>Status</th><th>Summary</th><th>Date</th></tr></thead><tbody>${gates.map((item)=>`<tr><td>${escapeHtml(item.release_candidate)}</td><td><span class="pill">${escapeHtml(item.gate_status)}</span></td><td>${escapeHtml(item.decision_summary ?? "-")}</td><td>${escapeHtml(item.decided_at ?? item.created_at)}</td></tr>`).join("")}</tbody></table></div>` : `<p class="empty">No release candidate gates recorded yet.</p>`;
  document.querySelector("#expansionView").innerHTML = `<section class="grid two"><section class="panel"><div class="panel-heading"><h2>Controlled Pilot Expansion</h2><p>Stage 18 tenant onboarding and release-candidate governance.</p></div>${renderRelatedContext("Expansion summary", [["Status", summary.status ?? "STAKEHOLDER_APPROVAL_REQUIRED"], ["Cohorts", summary.counts?.expansion_cohorts ?? cohorts.length], ["Approved cohorts", summary.counts?.approved_cohorts ?? countBy(cohorts,(item)=>item.expansion_status==="APPROVED")], ["Onboarding plans", summary.counts?.onboarding_plans ?? plans.length], ["Completed plans", summary.counts?.completed_onboarding_plans ?? countBy(plans,(item)=>item.onboarding_status==="COMPLETE")], ["Approved RC gates", summary.counts?.approved_release_gates ?? countBy(gates,(item)=>item.gate_status==="APPROVED")]])}</section><section class="panel compact-form"><div class="panel-heading"><h2>Expansion Cohort</h2><p>Create and approve a limited pilot cohort.</p></div><form id="expansionCohortForm"><label>Stakeholder decision<select name="stakeholder_decision_id"><option value="">None</option>${expansionDecisionOptions}</select></label><label>Cohort name<input name="cohort_name" value="Controlled Expansion Cohort 01" required /></label><label>Max tenants<input name="max_tenants" type="number" min="1" value="1" /></label><label>Max users<input name="max_pilot_users" type="number" min="1" value="5" /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Create Cohort</button>${disabledHint(Boolean(latestTenant), "Create a tenant before expansion.")}<p class="form-note">Endpoint: POST /pilot/expansion-cohorts</p></form><form id="expansionApproveForm"><label>Cohort<select name="expansion_cohort_id">${cohortOptions}</select></label><button type="submit" ${cohorts.length ? "" : "disabled"}>Approve Cohort</button><p class="form-note">Endpoint: POST /pilot/expansion-cohorts/update</p></form></section></section><section class="grid two"><section class="panel compact-form"><div class="panel-heading"><h2>Tenant Onboarding</h2><p>Create and complete onboarding plan for the cohort.</p></div><form id="onboardingPlanForm"><label>Cohort<select name="expansion_cohort_id"><option value="">None</option>${cohortOptions}</select></label><button type="submit" ${latestTenant ? "" : "disabled"}>Create Onboarding Plan</button><p class="form-note">Endpoint: POST /tenant-onboarding/plans</p></form><form id="onboardingCompleteForm"><label>Plan<select name="onboarding_plan_id">${planOptions}</select></label><button type="submit" ${plans.length ? "" : "disabled"}>Complete Onboarding</button><p class="form-note">Endpoint: POST /tenant-onboarding/plans/update</p></form></section><section class="panel compact-form"><div class="panel-heading"><h2>Release Candidate Gate</h2><p>Record explicit RC governance decision.</p></div><form id="rcGateForm"><label>Cohort<select name="expansion_cohort_id"><option value="">None</option>${cohortOptions}</select></label><label>Release candidate<input name="release_candidate" value="RC-STAGE-18-PILOT" /></label><label>Status<select name="gate_status"><option value="APPROVED">Approved</option><option value="HOLD">Hold</option><option value="REJECTED">Rejected</option><option value="PENDING">Pending</option></select></label><label>Summary<input name="decision_summary" value="Release candidate approved for controlled pilot expansion." /></label><button type="submit" ${latestTenant ? "" : "disabled"}>Record RC Gate</button><p class="form-note">Endpoint: POST /release-candidate/gates</p></form></section></section><section class="grid two"><section class="panel"><div class="panel-heading"><h2>Expansion Cohorts</h2><p>Controlled cohort limits and status.</p></div>${cohortRows}</section><section class="panel"><div class="panel-heading"><h2>Release Candidate Gates</h2><p>RC governance decisions.</p></div>${gateRows}</section></section>`;
  document.querySelector("#expansionCohortForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Create expansion cohort",form:event.currentTarget,success:"Expansion cohort created",action:async()=>{ const data=await request("/pilot/expansion-cohorts",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,stakeholder_decision_id:fd.get("stakeholder_decision_id")||null,cohort_name:fd.get("cohort_name"),max_tenants:Number(fd.get("max_tenants")),max_pilot_users:Number(fd.get("max_pilot_users")),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("expansion"); return data; }}); });
  document.querySelector("#expansionApproveForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Approve expansion cohort",form:event.currentTarget,success:"Expansion cohort approved",action:async()=>{ const data=await request("/pilot/expansion-cohorts/update",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,expansion_cohort_id:fd.get("expansion_cohort_id"),expansion_status:"APPROVED",actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("expansion"); return data; }}); });
  document.querySelector("#onboardingPlanForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Create onboarding plan",form:event.currentTarget,success:"Onboarding plan created",action:async()=>{ const data=await request("/tenant-onboarding/plans",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,expansion_cohort_id:fd.get("expansion_cohort_id")||null,actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("expansion"); return data; }}); });
  document.querySelector("#onboardingCompleteForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Complete onboarding",form:event.currentTarget,success:"Onboarding completed",action:async()=>{ const data=await request("/tenant-onboarding/plans/update",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,onboarding_plan_id:fd.get("onboarding_plan_id"),onboarding_status:"COMPLETE",actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("expansion"); return data; }}); });
  document.querySelector("#rcGateForm")?.addEventListener("submit",async(event)=>{ event.preventDefault(); const fd=new FormData(event.currentTarget); await runUiCommand({label:"Record RC gate",form:event.currentTarget,success:"Release candidate gate recorded",action:async()=>{ const data=await request("/release-candidate/gates",{method:"POST",body:JSON.stringify({tenant_id:latestTenant.id,firm_id:latestFirm?.id,expansion_cohort_id:fd.get("expansion_cohort_id")||null,release_candidate:fd.get("release_candidate"),gate_status:fd.get("gate_status"),decision_summary:fd.get("decision_summary"),actor:principalActor??systemActorForBrowser(latestTenant.id,latestFirm?.id)})}); await refresh(); switchView("expansion"); return data; }}); });
}
...
function renderRecordViews(store) {
  ...
  safeRenderModule("#frontDeskView", "Front Desk", () => renderIfSubscribed("#frontDeskView", "Front Desk", "front_desk", renderFrontDeskModule, store), store);
  safeRenderModule("#administrationView", "Administration", () => renderIfSubscribed("#administrationView", "Administration", "administration", renderAdministrationModule, store), store);
  safeRenderModule("#salesAccountsView", "Sales & Accounts", () => renderIfSubscribed("#salesAccountsView", "Sales & Accounts", "sales_accounts", renderSalesAccountsModule, store), store);
  safeRenderModule("#technicalDeliveryView", "Technical Delivery", () => renderIfSubscribed("#technicalDeliveryView", "Technical Delivery", "technical_delivery", renderTechnicalDeliveryModule, store), store);
  safeRenderModule("#proposalsView", "Proposals", renderProposalModule, store);
  safeRenderModule("#projectsView", "Projects", () => renderIfSubscribed("#projectsView", "Projects", "projects", renderProjectModule, store), store);
  safeRenderModule("#approvalsView", "Approvals", () => renderIfSubscribed("#approvalsView", "Approvals", "approvals", () => renderRecordView({ target: "#approvalsView", title: "Approvals", description: "Explicit approval decisions.", records: store.approvals ?? [], empty: "No approvals yet.", columns: [{ label: "Subject", value: (r) => r.subject_type }, { label: "Decision", value: (r) => r.decision }, { label: "Auth", value: (r) => r.authentication_strength }, { label: "ID", value: (r) => shortId(r.id) }] }), store), store);
  safeRenderModule("#invoicesView", "Invoices", () => renderIfSubscribed("#invoicesView", "Invoices", "invoices", renderInvoiceModule, store), store);
  safeRenderModule("#aiWorkforceView", "AI Workforce", () => renderIfSubscribed("#aiWorkforceView", "AI Workforce", "ai_workforce", renderAiWorkforceModule, store), store);
  safeRenderModule("#networkView", "Network", () => renderIfSubscribed("#networkView", "Network", "network", renderNetworkModule, store), store);
  safeRenderModule("#opsView", "Ops", () => renderIfSubscribed("#opsView", "Ops", "ops", renderOpsModule, store), store);
  safeRenderModule("#auditView", "Audit", () => renderIfSubscribed("#auditView", "Audit", "audit", renderAuditModule, store), store);
  safeRenderModule("#servicePackView", "Service Pack", renderServicePackModule, store);
  ...
  safeRenderModule("#reviewBoardView", "Review Board", renderReviewBoardModule, store);
  safeRenderModule("#expansionView", "Expansion", renderExpansionModule, store);
  safeRenderModule("#usageBillingView", "Usage/Billing", renderUsageBillingModule, store);
  safeRenderModule("#commercialLaunchView", "Commercial Launch", renderCommercialLaunchModule, store);
}
...
async function loadWorkspaceData() {
  try {
    const [summary, pairs] = await Promise.all([
      ...
      Promise.all(workspaceCollections.map(async ([key, path]) => [key, await request(path)]))
    ]);
...
async function checkApi() {
  try {
    const res = await fetch(`${API_BASE}/health`, { cache: "no-store" }); const health = await res.json();
    if (!res.ok || !health.ok) throw new Error(health.error?.message ?? "Health check failed");
    apiStatus.textContent = `${health.phase} on ${health.api_port ?? 3091}`; apiStatus.style.color = "var(--accent-dark)";
...
refreshStore.addEventListener("click", () => runUiCommand({ label: "Refresh workspace", button: refreshStore, success: "Workspace refreshed", action: () => refresh() }));

renderAll();
await checkApi();
await refresh().catch((error) => { setCommandFeedback("error", "Workspace render failed", commandErrorMessage(error)); setOperatorMessage("Workspace render failed. See diagnostic banner.", "danger"); });

import { forbiddenAiAuthorityActions, validateFactoryBlueprintBundle, type FactoryBlueprintBundleContract, type ValidationFinding } from "./factory-blueprints.js";

export interface ProfessionalAuthorityCertificationInput {
  valid: boolean;
  membership?: { id: string } | null;
  professional_profile?: { id: string } | null;
  professional_authority?: { id: string; permitted_actions?: string[]; credential_refs?: string[] } | null;
}

export interface WorkerBindingCertificationInput {
  id: string;
  worker_code: string;
  actor_type: string;
  binding_state: string;
  supervisor_actor_id?: string | null;
  authority_envelope?: { permitted_actions?: string[] } | null;
}

export interface PackBindingCertificationResult {
  ok: boolean;
  certification_state: "CERTIFIED" | "DENIED";
  compatibility_status: "PASS" | "FAIL";
  findings: Array<ValidationFinding & { source?: string }>;
  activated_services: Array<{ service_id: string; status: "ACTIVE" | "BLOCKED"; risk_class: string; responsible_professional_id: string | null; jurisdiction: string | null }>;
  authority_summary: { valid: boolean; professional_authority_id: string | null; professional_profile_id: string | null; membership_id: string | null };
  worker_binding_count: number;
}

const has = (value: unknown) => value !== undefined && value !== null && value !== "";

function addFinding(findings: Array<ValidationFinding & { source?: string }>, code: string, message: string, path = "$.", severity: "ERROR" | "WARNING" = "ERROR") {
  findings.push({ code, message, path, severity });
}

export function evaluatePackBindingCertification({ bundle, professionalAuthority, workerBindings = [] }: { bundle?: FactoryBlueprintBundleContract; professionalAuthority?: ProfessionalAuthorityCertificationInput | null; workerBindings?: WorkerBindingCertificationInput[] } = {}): PackBindingCertificationResult {
  const findings: Array<ValidationFinding & { source?: string }> = [];
  ...
  for (const [index, service] of (firm.services ?? []).entries()) {
  ...
  for (const [index, binding] of workerBindings.entries()) {
    ...
    for (const action of binding.authority_envelope?.permitted_actions ?? []) {
...
function buildResult(bundle: FactoryBlueprintBundleContract | undefined, findings: Array<ValidationFinding & { source?: string }>, professionalAuthority: ProfessionalAuthorityCertificationInput | null | undefined, workerBindings: WorkerBindingCertificationInput[]): PackBindingCertificationResult {
  ...
  return {
    ...
    findings,
    activated_services: (bundle?.firm_blueprint?.services ?? []).map((service) => ({ service_id: service.service_id, status: ok ? "ACTIVE" : "BLOCKED", risk_class: service.risk_class ?? "STANDARD", responsible_professional_id: service.responsible_professional_id ?? null, jurisdiction: service.jurisdiction ?? null })),
    authority_summary: { valid: Boolean(professionalAuthority?.valid), professional_authority_id: professionalAuthority?.professional_authority?.id ?? null, professional_profile_id: professionalAuthority?.professional_profile?.id ?? null, membership_id: professionalAuthority?.membership?.id ?? null },
